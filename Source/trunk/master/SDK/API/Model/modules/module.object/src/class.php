 ...
 * @updated	December 18, 2015, 12:55 (GMT)
 */
class module
{
	/**
	...
	/**
	 * All module key types.
	 * 
	 * @type	array
	 */
	private static $allKeyTypes = array();
	...
	/**
	 * Get all module available key types.
	 * 
	 * @return	array
	 * 		An array of type id and type name.
	 * 		id => type.
	 */
	public static function getAllKeyTypes()
	{
		// Check cache
		if (!empty(self::$allKeyTypes))
			return self::$allKeyTypes;
		
		// Get Module Detail Info
		$dbc = new dbConnection();
		$dbq = new dbQuery("21252084428319", "security.privileges.keys");
		$result = $dbc->execute($dbq);
		return self::$allKeyTypes = $dbc->toArray($result, "id", "type");
	}
	...
	public static function getKeyTypes($moduleID)
	{
		...
		while ($t = $dbc->fetch($result))
			self::$moduleKeyTypes[$moduleID][] = $t['key_type_id'];
		
		return self::$moduleKeyTypes[$moduleID];
	}
	
	/**
	 * Update the module key types.
	 * 
	 * @param	integer	$moduleID
	 * 		The module id to update the key types.
	 * 
	 * @param	array	$keyTypeIDs
	 * 		An array including all the key type ids for the module.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public static function updateKeyTypes($moduleID, $keyTypeIDs)
	{
		$dbc = new dbConnection();
		$attr = array();
		$attr['id'] = $moduleID;
		...
		// Clear key types
		$dbq = new dbQuery("29143166176073", "security.privileges.keys");
		$dbc->execute($dbq, $attr);
		
		// Add module key types
		$dbq = new dbQuery("31774644857842", "security.privileges.keys");
		foreach ($keyTypeIDs as $typeID)
		{
			$attr['type'] = $typeID;
			$dbc->execute($dbq, $attr);
		}
	...
	/**
	 * Update module scope and status.
	 * 
	 * @param	integer	$moduleID
	 ...
	 * 		The module status id.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public static function updateAccessInfo($moduleID, $scopeID, $statusID)
	{
		// Update module access info
		$dbc = new dbConnection();
		$dbq = new dbQuery("17683760949934", "modules");
		
		// Get Module Group Data
		$attr = array();
		$attr['id'] = $moduleID;
		$attr['scope'] = $scopeID;
		$attr['status'] = $statusID;
