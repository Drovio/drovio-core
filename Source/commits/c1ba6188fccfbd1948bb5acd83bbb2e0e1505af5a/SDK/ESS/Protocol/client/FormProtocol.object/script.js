...
FormProtocol = {
	init : function() {
		...
		jq(document).on('submited.form', 'form', function(ev) {
			jq(this).removeData("preventUnload");
		});
		
		// Get form event listeners
		jq(document).off('reset.form', 'form');
		jq(document).on('reset.form', 'form', function(ev) {
			FormProtocol.reset(ev, jq(this));
		});
	},
	...
	submit : function(ev, jqForm) {
		...
		if (jqForm.data("form-action") != undefined) {
			...
			ascop.asyncRequest(formAction, "POST", formInputs, "json", jqForm, handlerFunction, null, false);
		}
	},
