...
class RCPage extends HTMLPageTemplate
{
	 ...
	 * 		The page object.
	 */
	public function build($subdomain = "", $static = FALSE, $meta = FALSE)
	{
	...
	private function populateBody($subdomain, $static)
	{
		...
		$toolbar = new RNavToolbar($subdomain);
	...
	private function getPageNotification($access)
	{
		...
		switch ($access)
		{
			case "auth":
				// Build Notification
				$ntf = $notification->build($type = "error", $header = TRUE, $timeout = FALSE, $disposable = FALSE)->get();
