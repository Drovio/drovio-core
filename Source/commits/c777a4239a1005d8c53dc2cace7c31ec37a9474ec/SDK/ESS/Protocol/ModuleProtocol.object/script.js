...
ModuleProtocol = {
	init: function() {
		// Self-triggered AJAX Content (loadContent)
		jq(document).on("loadContent", "[data-mdl-ajx]", function(ev, loading, extraParams) {
			// Stops the Bubbling
			ev.stopPropagation();
		...
		jq(document).on("reload", "[data-mdl-ajx]", function(ev, loading, extraParams) {
			// Stops the Bubbling
			ev.stopPropagation();
			
			// Load Content
			ModuleProtocol.trigger(ev, jq(this), "GET", "mdl-ajx", extraParams, null, null, null, loading);
		});
	...
	contentModified : function() {
		...
		// Load listener to module protocol
		jq("[data-mdl-ajx]").off("load");
		jq("[data-mdl-ajx]").on("load", function(ev, loading, extraParams) {
			// Stops the Bubbling
			ev.stopPropagation();
	...
	downloadRequest: function(jqSender, extraParams, callType) {
		...
		var downloadUrl = "/ajax/modules/load.php?" + requestData;
