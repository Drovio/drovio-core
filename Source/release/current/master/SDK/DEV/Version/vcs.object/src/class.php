 ...
 * 
 * @version	4.0-2
 * @created	February 12, 2014, 9:55 (GMT)
 * @updated	January 6, 2016, 11:37 (GMT)
 */
class vcs
{
	/**
	 ...
	 * 
	 * @param	string	$tags
	 * 		A list of tags for this commit in a string form.
	 * 		Tags are comma separated by.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure or if there are no items to commit.
	 */
	public function commit($summary, $description = "", $commitItems = array(), $tags = "")
	{
		$finalCommit = FALSE;
		...
		$attr['time'] = $timestamp;
		$attr['author'] = $this->getAuthorID();
		$attr['summary'] = trim($summary);
		$attr['description'] = trim($description);
		$tags = trim($tags);
		$tags = trim($tags, ",");
		$attr['tags'] = strtolower($tags);
		$result = $this->dbc->execute($dbq, $attr);
	 ...
	 * 
	 * @return	mixed
	 * 		The release folder or FALSE in case of an error.
	 */
	...
	public function setWorkingBranch($branchName)
	{
		...
		if (empty($authorID))
			Throw New Exception("There is no author defined for this project [".$this->projectID."].");
	...
	public function getBranchCommits($branchName = self::MASTER_BRANCH)
	{
		...
		while ($commit = $this->dbc->fetch($result))
		{
			...
			if (array_key_exists($authorID, $authors)) {
				...
				$authors[$authorID] = $authorInfo;
			}
			
			// Add authro info to commit info
			$commit['author'] = $authorInfo['title'];
	 ...
	 * 		item[itemID][author-id]
	 * 		item[itemID][author-title]
	 * 		item[itemID][path]
	 * 		item[itemID][force_commit]
	 * 		item[itemID][last-edit-author-id]
	 * 		item[itemID][last-edit-author-title]
	 * 		item[itemID][last-edit-time].
	 */
	public function getWorkingItems()
	{
		// Get all working items
		$workingBranch = $this->getWorkingBranch();
		$allWorkingItems = $this->getAllWorkingItems($workingBranch);
		
		// Filter only for current account
		$currentAuthorID = $this->getAuthorID();
		$workingItems = array();
		foreach ($allWorkingItems as $index => $info)
			if ($info['author-id'] == $currentAuthorID)
				$workingItems[$index] = $info;
		
		return $workingItems;
	}
	
	/**
	 * Get all working items for the current project in the given branch.
	 * 
	 * @param	string	$branchName
	 * 		The branch name to get working items for.
	 * 
	 * @return	array
	 * 		An array of items as follows:
	 * 		item[itemID][author-id]
	 ...
	 * 		item[itemID][last-edit-time].
	 */
	public function getAllWorkingItems($branchName = self::MASTER_BRANCH)
	{
		// Get working items
		$dbq = new dbQuery("34725562948977", "developer.vcs", TRUE);
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $branchName;
		$result = $this->dbc->execute($dbq, $attr);
		...
		foreach ($items as $item)
		{
			...
			// Get initial author info
			$initialAuthorID = $item['author_id'];
			$initialAuthorInfo = account::info($initialAuthorID);
			$itemInfo['author-id'] = $item['author_id'];
			$itemInfo['author-title'] = $initialAuthorInfo['title'];
			
			// Get last author info
			$lastAuthorID = $item['last_author_id'];
			$lastAuthorInfo = account::info($lastAuthorID);
			$itemInfo['last-edit-author-id'] = $item['last_author_id'];
			$itemInfo['last-edit-author-title'] = $lastAuthorInfo['title'];
			$itemInfo['last-edit-author'] = $lastAuthorInfo['title'];
	...
	/**
	 * Get all the project accounts and all the active authors (accounts with commits).
	 * 
	 * @return	array
	...
	public function getAuthors()
	{
		...
		// Get active accounts
		$dbq = new dbQuery("33392573164862", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$result = $this->dbc->execute($dbq, $attr);
		$activeAccounts = $this->dbc->fetch($result, TRUE);
		
		// Merge accounts
		$totalAuthors = array();
		foreach ($projectAccounts as $account)
			$totalAuthors[$account['accountID']] = $account['title'];
