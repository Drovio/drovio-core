...
jq(document).one("ready.extra", function() {
	jq(document).on("content.modified", function(){
		...
		gLists.find(".uiDataGridList .dataGridRow > .dataGridCheck :checked")
		...
		gLists.find(".gridList").each(function(){
			var jqthis = jq(this);
	...
	jq(document).on("click", ".uiDataGridList > .dataGridHeader > .dataGridCell", function(){
		jq(".uiDataGridList > .dataGridHeader > .dataGridCell")
	...
	// sort on click
	jq(document).on("click", ".uiDataGridList > .dataGridHeader > .dataGridCell", function(){
		jqthiscol = jq(this).closest(".uiDataGridList")
							.find(".dataGridRow")
		...
		var jqthisrow = jq(this).closest(".uiDataGridList").find(".dataGridRow");
	...
	jq(document).on("change", ".uiDataGridList .dataGridRow > .dataGridCheck [type='checkbox']", function(ev){
		...
		var glist = thisCheck.closest(".uiDataGridList");
		var rows = glist.find(".dataGridRow");
		var checks = rows.find(".dataGridCheck [type='checkbox']");
	...
	jq(document).on("reset", "form:has(.uiDataGridList)", function(ev){
		var jqthis = jq(this);
		setTimeout(function() {
			jq(".uiDataGridList .dataGridRow > .dataGridCheck [type='checkbox']", jqthis)
	...
	/*
	jq(document).on("deactivateRow", ".uiDataGridList", function(ev, index) {
		jq(this).find(".dataGridContentWrapper .dataGridRow").eq(index).addClass("inactive");
	});
	
	jq(document).on("activateRow", ".uiDataGridList", function(ev, index) {
