 ...
 * @updated	February 1, 2015, 19:05 (EET)
 */
	...
	public function publish($version, $branchName = vcs::MASTER_BRANCH)
	{
		...
		foreach ($allViews as $folderName => $views)
			foreach ($views as $viewName)
			{
				...
				$viewHTML = str_replace("%resources%", $resourceUrl, $viewHTML);
				$viewHTML = str_replace("%{resources}", $resourceUrl, $viewHTML);
				$viewHTML = str_replace("%media%", $resourceUrl, $viewHTML);
				$viewHTML = str_replace("%{media}", $resourceUrl, $viewHTML);
				// Create file
		...
		// Publish css file
		BootLoader2::exportCSS($appID, "Apps", $appID, $cssContent, $version);
		...
		BootLoader2::exportJS($appID, "Apps", $appID, $jsContent, $version);
