...
BootLoader = {
		...
		for (key in report.head) {
			...
			if (header.header_type == "rsrc") {
				...
				if (hasJS) {
					...
					BootLoader.loadResource(rsrcID, "js", rsrcUrl, header, callback);
	...
	loadCSSResource : function(rsrcID, rsrcUrl, attributes) {
		...
		// Load Resource
		var cssUrl = rsrcUrl+"?"+attrParams.join("&");
	...
	loadCSS : function(href) {
		return jq("<link rel='stylesheet' href='"+ascop.resolve(url.resource(href))+"'>").appendTo(jq("head"));
	}
};