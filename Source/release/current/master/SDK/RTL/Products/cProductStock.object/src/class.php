 ...
 * @updated	September 24, 2015, 0:56 (EEST)
 */
class cProductStock
{
	/**
	 ...
	 * 		True on success, false on failure.
	 * 
	 * @deprecated	Use cProduct::setMUnit() instead.
	 */
	public function setMUnit($mUnit)
	{
		$product = new cProduct($this->productID);
		return $product->setMUnit($mUnit);
	}
	...
	public static function getMeasurementUnits()
	{
		...
		$units = array();
		$tliterals = literal::get($scope = "retail.stock.units", $name = "", $attr = array(), $wrapped = FALSE);
		while ($row = $dbc->fetch($result))
			$units[$row['id']] = $tliterals["unit_".$row['title']];
