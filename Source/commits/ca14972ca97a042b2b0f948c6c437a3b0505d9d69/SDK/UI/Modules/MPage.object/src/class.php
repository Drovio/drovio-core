 ...
 * @updated	February 19, 2015, 12:32 (EET)
 */
class MPage extends MContent
{
	 ...
	 */
	public function getRCollection($id, $title = "", $moduleID = "", $viewName = "", $startup = FALSE)
	{
		return RTRibbon::createCollection($id, $title, $moduleID, $viewName, $startup);
	}
	...
	public function getReport($holder = "", $clearToolbar = TRUE)
	{
		...
		if (!empty($this->title))
		{
			...
			// Or set action to update the title from javascript async
			$headTitle = HTML::select("head title")->item(0);
			if (!empty($headTitle))
				HTML::innerHTML($headTitle, $this->title);
			else
