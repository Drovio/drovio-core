 ...
 * @updated	January 10, 2015, 20:06 (EET)
 */
class ftp
{
	 ...
	 * 
	 * @param	string	$dName
	 * 		The remote file name. If $dName is empty the file name assumed to be concacated with the filepath ($dPath)
	 * 
	 * @return	boolean
	 * 		True on Success, False elsewhere
	 */
	public function write($dPath, $contents = '', $dName = '')
	{
		$mode = FTP_ASCII;
		//
		if(!empty($dName))
		...
		// write the file
		if (ftp_fput($this->connID, $dPath, $stream, $mode))
		{
			logger::log("Content have been written to ".$dPath, logger::INFO);
			return TRUE;
		} 
		else
		{
			logger::log("Could not write file to ".$dPath, logger::ERROR);
			return FALSE;
		}
	...
	public function getDirContents($dir = '.', $parameters = '-la')
	{
		...
		$contentsArray = ftp_nlist($this->connID, $parameters . '  ' . $dir);
	...
	/**
	 * Tries to change the current ftp working directory. If the createDir flag is TRUE, the target directory will be created if is not exists and then it will be set as current.
	 * 
	 * @param	string	$dir
	 * 		The target directory
	 * 
	 * @param	boolean	$createDir
	 * 		A flag indicating wherever the target dir will be created is it does not exist. Default is FALSE.
	 * 
	 * @return	boolean
	 * 		True on Success, False elsewhere
	 */
	public function changeDir($dir, $createDir = FALSE)
	{ 
		// If changing directory fails, PHP will throw a warning.
        	$status = ftp_chdir($this->connID, $dir);
        	if(!$status)
        	{
			// If failed to change the current directory
				...
				// Attempt to create possible missing target dir
		                logger::log("Attempting to create possible missing target dir: ".$dir, logger::INFO);
				if($this->makeDir($dir))
					return $this->changeDir($dir, FALSE);
			}
            		logger::log("To failed to navigate to ".$dir, logger::ERROR);
			return FALSE;
        	}
        	return $status;
	}
