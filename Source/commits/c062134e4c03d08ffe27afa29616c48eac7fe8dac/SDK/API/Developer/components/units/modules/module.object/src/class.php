 ...
 * @revised	December 6, 2013, 11:06 (EET)
 */
class module
{
	/**
	...
	/**
	 * Get old module content and migrate to the new repository.
	 * 
	 * @return	void
	 */
	...
	{
		// Load old module
		$moduleObject = new moduleObject($this->id);
		$module = $moduleObject->getModule();
		$phpCode_old = $module->getSourceCode();
		$cssCode_old = $module->getCSSCode();
		$jsCode_old = $module->getJSCode();
		$imports = $module->getImports();
		$inner = $module->getInnerModules();
		$dependencies = array();
		foreach ($imports as $library => $packages)
		...
		// Copy main view code
		$moduleView = $this->getView();
		$moduleView->updateInfo($dependencies, $inner);
		$moduleView->updateView("", $cssCode_old);
		$moduleView->updatePHPCode($phpCode_old);
		$moduleView->updateJSCode($jsCode_old);
		
		// Get auxiliaries and create new views
		$auxiliaries = $module->getAuxiliaries();
		foreach ($auxiliaries as $seed => $auxTitle)
		{
			// Get auxiliary
			$moduleAux = $moduleObject->getModule($auxTitle);
			$phpCode_old = $moduleAux->getSourceCode();
			$imports = $moduleAux->getImports();
			$inner = $moduleAux->getInnerModules();
			$dependencies = array();
			foreach ($imports as $library => $packages)
				foreach ($packages as $package)
					$dependencies[$library.",".$package] = "";

			// Create new view and copy source code
			$this->createView($auxTitle);
			$auxView = $this->getView($auxTitle);
			$auxView->updateInfo($dependencies, $inner);
			$auxView->updatePHPCode($phpCode_old);
		}
	}
}
//#section_end#
