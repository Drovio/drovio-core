 ...
 * @revised	November 18, 2013, 8:22 (EET)
 */
class vcs
{
	/**
	 ...
	 */
	public function commit($description, $commitItems = array())
	{
		$finalCommit = FALSE;
		...
		$commitElement = $parser->create("commit", $description, $commitID);
	...
	/**
	 * Creates a release of this project.
	 * 
	 * @param	string	$versionName
	 * 		The version name for this release.
	 * 
	 ...
	 * @return	void
	 */
	public function release($versionName, $description)
	{
		// Set version id
		$versionID = "v".$versionName;
		$versionTime = time();
		...
		$release = $parser->find($versionID);
		if (!is_null($release))
			return FALSE;
			
		$release = $parser->create("release", $description, $versionID);
		$parser->attr($release, "time", $versionTime);
		$parser->append($root, $release);
		// __ update file
		$parser->update();
		
		// Get Head Branch and its folder
		$headBranch = $this->getHeadBranch();
		$headBranchFolder = $this->repository.self::BRANCHES_FOLDER.$headBranch;
		
		// Copy files to current version
		$currentVersionFolder = $this->repository.self::RELEASE_FOLDER.self::CURRENT_VERSION_FOLDER;
		folderManager::clean($currentVersionFolder);
		folderManager::copy($headBranchFolder, $currentVersionFolder, TRUE);
		
		// Create version folder (temp, for zip only)
		$versionFolder = $this->repository.self::RELEASE_FOLDER.$versionID;
		folderManager::create($versionFolder);
		folderManager::copy($currentVersionFolder, $versionFolder, TRUE);
		
		// Create zip version zipManager
		$releaseZipName = $this->repository.self::RELEASE_FOLDER.self::HISTORY_FOLDER.$versionID.".zip";
		$releaseContents = directory::getContentList($versionFolder."/", TRUE);
		zipManager::create($releaseZipName, $releaseContents, TRUE, TRUE);
		folderManager::remove($versionFolder, "", TRUE);
	...
	public function setWorkingBranch($branchName)
	{
		...
		if (empty($branchName) || !in_array($branchName, $branches))
			return FALSE;
	...
	public function getBranches()
	{
		...
		foreach ($branches as $branch)
			$info[] = $parser->attr($branch, "id");
		
		return $info;
	}
	
	/**
	 * Gets all the releases of this repository
	 * 
	 * @return	array
	...
	public function getReleases()
	{
		...
		{
			// Get release id
			$releaseID = $parser->attr($release, "id");
			
			// Create release info
			$releaseInfo = array();
			$releaseInfo['time'] = $parser->attr($release, "time");
			$releaseInfo['description'] = $parser->nodeValue($release);
			
			// Update
			$info[$releaseID] = $releaseInfo;
		}
