...
$counter = 0;
		...
		{
			$moduleObj = new moduleObject($row['id']);
			...
			if (!empty($imports))
			{
				...
				$moduleNode = $importsParser->create("module", "", "m.".$row['id']);
			...
			// Get Inner Modules
			$innerModules = $module->getInnerModules();
			if (!empty($innerModules))
			{
				$moduleNode = $innerParser->create("module", "", "m.".$row['id']);
			...
			// Get auxiliaries
			$auxs = $module->getAuxiliaries();
			foreach ($auxs as $auxTitle)
			{$counter++;
				$moduleAux = $moduleObj->getModule($auxTitle);
				$moduleAux->export();
			}
		}
