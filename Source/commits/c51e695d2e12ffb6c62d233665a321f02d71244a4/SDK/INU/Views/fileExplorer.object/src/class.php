 ...
 * @revised	September 18, 2013, 19:28 (EEST)
 */
class fileExplorer extends UIObjectPrototype
{
	 ...
	 */
	public function getFileArea($subPath = "")
	{
		...
		{	
			...
			$dtGridList->insert_row($gridRow, "files[]", FALSE);
		}
		...
		{	
			...
			if (strstr($mimeType, "svg") !== FALSE)
			{
				...
				DOM::attr($embed, "src", $this->rootPath."/".$subPath."/".$details['name']);
	...
	public function packFiles($fileNames)
	{
		...
		{
			// Just locate a single file...
			$zipname = $fileNames[0].".zip";
		}
		else
		{
			// Pack files in zip and locate that zip...
			$zipname = basename($this->rootPath).".zip";
		}
		$random = rand();
		$archive = sys_get_temp_dir()."/fileExplorer_".$random.".zip";
		...
		$status = zipManager::create($archive, $files, FALSE, TRUE);
		
		$info = array();
		$info['name'] = $archive;
		$info['suggest'] = $zipname;
		$info['info'] = $status;
	 ...
	 * @return	void
	 */
	private function getDOMRepresentationIcon($fileName, $subPath)
	{
		/*$wrap = DOM::create("div", "", "", "previewWrapper");
	...
	{
		$stateWrapper = DOM::create("div", "", "", "stateViewer");
		
		if (is_string($span))
			$span = DOM::create("span", $span);
			
		DOM::append($stateWrapper, $span);
