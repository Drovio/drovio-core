...
//	return;	
	...
	jq(document).off("content.modified.csseditor");
	...
	(function(jq){
		...
		// Function logic for the preview select
		var browserStyles = new Object();
		browserStyles.active = restorePreviewStyles;
		browserStyles.w3c = setPreviewStyles;
		browserStyles.firefox = setPreviewStyles;
		browserStyles.opera = setPreviewStyles;
		browserStyles.webkit = setPreviewStyles;
		browserStyles.iexplorer = setPreviewStyles;
		...
		jq.fn.cssEditor = function(/*method*/) {
		...
		*/
			...
			cssEditor.on("click.csseditor", ".previewBar .toggleHTMLEditor", function(ev){
				var jqbar = jq(this).closest(".previewBar");
				var me = jqbar.siblings(".modelEditor");
				jqbar.siblings(".previewContainer, .modelEditor").toggleClass("noDisplay");
				var txt = jq(this).text();
				var toggleName = jq(this).data("toggle-name");
				jq(this).children().text(toggleName);
				jq(this).data("toggle-name", txt);
				...
				if (coder == "true") {
					...
					htmlCode.trigger("getCodeEditorContent", [true, function(content){
						...
						// Filter pool
						/*for (var selector in templateCollection) {
							if (jq(selector, structure).length == 0)
								jq(this).remove();
						}*/
