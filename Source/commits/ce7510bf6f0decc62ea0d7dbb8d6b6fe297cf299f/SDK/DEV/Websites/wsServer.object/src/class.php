 ...
 * @revised	January 2, 2015, 12:30 (EET)
 */
class wsServer
{
	/**
	...
	public function create($name, $address, $connectionType = self::CON_TYPE_FTP)
	{
		...
		// Add properties
		$connectionType = (empty($connectionType) ? self::CON_TYPE_FTP : $connectionType);
	 ...
	 * 		For more information see the class' constants.
	 * 
	 * @param	string	$httpdocs
	 * 		The root folder on server where the website will be installed in and ger served from.
	 * 
	 * @param	string	$webroot
	 * 		The trailing path of server url under where the site will be installed
	 * 
	 * @return	boolean
	 * 		True on success, false on failure
	 */
	public function updateInfo($description, $serverType, $connectionType = self::CON_TYPE_FTP, $httpdocs = "httpdocs", $webroot="")
	{
		$parser = $this->xmlParser;
		...
		$this->setProperty($server, "httpdocs", $httpdocs);
		$this->setProperty($server, "webroot", $webroot);
	 ...
	 * 		- password.
	 */
	public function getProductionServerInfo()
	{
		return $this->getServerInfo($this->serverID);
	}
	...
	private function getServerInfo($serverID)
	{
		...
		$serverInfo['httpdocs'] = $this->getProperty($server, "httpdocs");
		$serverInfo['webroot'] = $this->getProperty($server, "webroot");
