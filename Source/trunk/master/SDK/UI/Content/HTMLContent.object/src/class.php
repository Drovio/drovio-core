 ...
 * 
 * @version	5.1-2
 * @created	July 30, 2014, 10:47 (BST)
...
class HTMLContent extends UIObjectPrototype
{
	 ...
	 * 
	 * @param	string	$method
	 * 		The report method. See class constants.
	 * 
	 * @return	void
	 */
	 ...
	 * 
	 * @param	boolean	$merge
	 * 		Whether to merge the given header with an existing value or not (replace).
	 * 		It is TRUE by default.
	 * 
	 * @return	void
	 */
	public function addHeader($header, $key = "", $merge = TRUE)
	{
		HTMLServerReport::addHeader($header, $key, $merge);
	}
	...
	public function addResourceHeader($rsrcID, $header)
	{
		// Set resource id
		$resource = array();
		$resource[$rsrcID] = $header;

		// Add resource header
		$this->addHeader($resource, BootLoader::RSRC_HD_KEY, $merge = TRUE);
	}
	 ...
	 */
	public function getReport($holder = "", $method = self::REPLACE_METHOD)
	{
		// Check version
		$method = ($method != self::APPEND_METHOD ? self::REPLACE_METHOD : self::APPEND_METHOD);
