 ...
 * @updated	November 22, 2015, 13:08 (GMT)
 */
abstract class webDoc
{
	...
	private function updateDocumentInfo($parser)
	{
		...
		// Get document author element
		$authorElement = $xmlParser->evaluate("/document/author")->item(0);
		if (empty($authorElement))
		{
			$authorElement = $xmlParser->create("author");
			$xmlParser->prepend($root, $authorElement);
		}
		// Update document author
		$xmlParser->attr($authorElement, "id", account::getInstance()->getAccountID());
		$xmlParser->nodeValue($authorElement, account::getInstance()->getAccountTitle());
		...
		// Get document sub element
		$subtitleElement = $xmlParser->evaluate("/document/subtitle")->item(0);
		if (empty($subtitleElement))
		{
			$subtitleElement = $xmlParser->create("subtitle");
			$xmlParser->prepend($root, $subtitleElement);
		}
		
		// Set document title value
		$docSubtitle = $parser->select("sub")->item(0);
		if (!empty($docSubtitle))
			$xmlParser->innerHTML($subtitleElement, $docSubtitle->nodeValue);
	...
	public function getDocumentInfo()
	{
		...
		$subtitleElement = $xmlParser->evaluate("/document/subtitle")->item(0);
		$documentInfo['subtitle'] = $subtitleElement->nodeValue;
		$documentInfo['time_created'] = $xmlParser->attr($root, "created");

		// Get document author
		$authorElement = $xmlParser->evaluate("/document/author")->item(0);
		$author = array();
		$author['id'] = $xmlParser->attr($authorElement, "id");
		$author['title'] = $authorElement->nodeValue;
