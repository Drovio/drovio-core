 ...
 * 
 * @version	1.0-2
 * @created	March 8, 2013, 8:42 (GMT)
 * @updated	October 30, 2015, 18:12 (GMT)
 */
class DOMParser extends DOMPrototype
{
	...
	public function select($selector, $context = NULL)
	{
		...
		$xpath = parent::CSSSelector2XPath($selector);

		// Get the context node if css context
		if (!empty($context) && is_string($context))
		{
			$ctxList = $this->select($context);
			if (empty($ctxList) || empty($ctxList->length))
			{
				logger::log("DOMParser->select(). Context node is not found.", logger::ERROR);
				return FALSE;
			}
	...
	public function save($path, $fileName = "", $format = FALSE)
	{
		...
		if (!file_exists($this->filePath))
			fileManager::create($this->filePath, "", TRUE);
