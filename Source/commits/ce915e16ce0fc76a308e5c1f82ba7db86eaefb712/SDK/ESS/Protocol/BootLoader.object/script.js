...
BootLoader = {
	...
	loadResources : function (report, callback) {
		...
		for (key in report) {
			var header = report[key];
			if (header.type == "rsrc") {
				var hasCSS = (jq.type(header.css) != "undefined");
				var hasJS = (jq.type(header.js) != "undefined");
				var rsrcID = header.id;
				if (hasCSS) {
					var cssUrl = header.css;
					var static = true;
					if (header.tester) {
						if (cssUrl == true || cssUrl.indexOf("http") < 0)
							cssUrl = "/ajax/resources/explicit/css.php";
						static = false;
					}
					else
						if (cssUrl.indexOf("http") < 0)
							cssUrl = "/Library/Resources/css/"+header.css+".css";
					BootLoader.loadResource(rsrcID, "css", cssUrl, header.attributes, null, static);
				}
				...
				if (hasJS) {
					...
					var static = true;
					if (header.tester) {
						if (jsUrl == true || jsUrl.indexOf("http") < 0)
							jsUrl = "/ajax/resources/explicit/js.php";
						static = false;
					}
					else
						if (jsUrl.indexOf("http") < 0)
							jsUrl = "/Library/Resources/js/"+header.js+".js";
					
					// Check if callback is already called
					if (callbackCalled)
						callback = null;
					BootLoader.loadResource(rsrcID, "js", jsUrl, header.attributes, callback, static);
					callbackCalled = true;
				}
			}
		}
