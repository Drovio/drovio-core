 ...
 * @updated	October 29, 2015, 11:48 (GMT)
 */
class accountSession
{
	 ...
	 * 		{description}
	 */
	public function create($salt, $personID = "", $rememberme = FALSE)
	{
		// STATIC COMPATIBILITY CHECK
		if (!(isset($this) && get_class($this) == __CLASS__))
			return accountSession::getInstance(self::$staticTeamName, self::$staticMxID)->create($salt, $personID, $rememberme);
		
		// Get account id
		$accountID = account::getInstance($this->teamName)->getAccountID();
	...
	public function info($sessionID = "")
	{
		...
		$sessionInfo['location'] = geoIP::getCountryCode2ByIP($sessionInfo['ip']);
	...
	public function getActiveSessions($accountID = "")
	{
		...
		{
			// Get extra location from ip
			$sinfo['location'] = geoIP::getCountryCode2ByIP($sinfo['ip']);
