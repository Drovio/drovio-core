...
importer::import("API", "Security", "akeys/apiKeyOrigin");
...
use \API\Profile\team;
use \API\Security\akeys\apiKeyOrigin;
 ...
 * @updated	December 18, 2015, 14:43 (GMT)
 */
class apiKey
{
	/**
	...
	public static function info($akey, $includePreviousKey = FALSE)
	{
		...
		$keyInfo = $dbc->fetch($result);
		
		// Add permission group name
		$keyInfo['user_group_name'] = $pmGroups[$keyInfo['user_group_id']];
	...
	public static function getAccountKeys($accountID = NULL)
	{
		...
		{
			$row['user_group_name'] = $pmGroups[$row['user_group_id']];
			$keys[] = $row;
		}
	...
	/**
	 * Get all keys that are connected to a given team id.
	 * 
	 * @param	integer	$teamID
	 * 		The team id to get keys for.
	 * 
	 * @return	array
	 * 		All api keys with their information.
	 */
	public static function getAllTeamKeys($teamID)
	{
		// Get permission groups
		$pmGroups = privileges::getPermissionGroups();
		...
		// Set Database Connection
		$dbc = new dbConnection();
		$q = new dbQuery("33474975700782", "security.akeys");
	...
	/**
	 * Get all api keys that are connected to the given project.
	 * 
	 * @param	integer	$projectID
	 * 		The project id to get the keys for.
	 * 
	 * @return	array
	 * 		An array of all API keys and their information.
	 */
	public static function getAllProjectKeys($projectID)
	{
		// Get permission groups
		$pmGroups = privileges::getPermissionGroups();
		...
		// Set Database Connection
		$dbc = new dbConnection();
		$q = new dbQuery("15785300909532", "security.akeys");
	...
	{
		return apiKeyOrigin::validate($akey, $origin);
	}
