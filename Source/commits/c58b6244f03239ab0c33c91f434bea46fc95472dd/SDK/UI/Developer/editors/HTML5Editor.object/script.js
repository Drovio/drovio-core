...
jq(document).one("ready", function(){
	...
	jq(document).on("content.modified", function(){
		// Initialize all HTML5Editors
		jq(".html5Editor").html5Editor();
	});
...
(function(jq){
	...
	function HTML5EditorInitialize() {
		HTML5Editor = jq(this);
		previewPanel = HTML5Editor.find(".previewPanel");
		htmlCodePanel = HTML5Editor.find(".htmlCodePanel");
		htmlCodeEditor = HTML5Editor.find(".htmlCodePanel .codeEditor");
		
		// Check initiated
		if (HTML5Editor.data("initiated"))
			return;
		
		// Initialized editor
		HTML5Editor.data("initiated", true);
	...
	function syncViews() {
		if (previewPanel.hasClass("noDisplay")) {
			...
			htmlCodeEditor.trigger("getCodeEditorContent", [true, function(htmlCode) {
				//htmlCode = collapseXml(htmlCode);
				previewPanel.html(htmlCode);
				//previewPanel.contents().not(previewPanel.children()).wrap("<p></p>");
			}]);
			...
			htmlCodeEditor.trigger("replaceContent", [htmlText]);
		}
	}
