 ...
 * @revised	January 20, 2014, 10:56 (EET)
 */
class releaseManager extends UIObjectPrototype
{
	...
	private function buildVersionSection()
	{
		...
		foreach ($branches as $branchData)
		{
			...
			$currentRelease = $releases[$branchData['name']]['current'].".".$releases[$branchData['name']]['build'];
			$currentRelease = (empty($currentRelease) ? "No Release" : $currentRelease);
			$rv = DOM::create("span", $currentRelease, "", "rV");
		...
		// Release version
		$input = $form->getInput($type = "text", $name = "version", $value = "", $class = "", $autofocus = FALSE, $required = TRUE);
