...
ApplicationProtocol = {
	init: function() {
		...
		jq(document).on("loadContent", "[data-app-ajx]", function(ev, loading) {
			// Stops the Bubbling
			ev.stopPropagation();
		...
		jq(document).on("reload", "[data-app-ajx]", function(ev, loading) {
			// Stops the Bubbling
			ev.stopPropagation();
			
			// Load Content
			ApplicationProtocol.request(jq(this), "GET", null, null, null, null, loading, "app-ajx");
		});
	...
	contentModified : function() {
		...
		jq("[data-app-ajx]").on("load", function(ev, loading) {
			// Stops the Bubbling
			ev.stopPropagation();
		...
		jq("[data-app-startup]").each(function(){
			// Remove attribute for startup and load content
			jq(this).data("startup", true).removeAttr("data-app-startup");
			jq(this).trigger("loadContent");
		});
