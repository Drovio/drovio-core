...
jq(document).one("ready.extra", function() {
	...
	jq(document).on("updateSourceModel.documentation", ".documentorContainer .documentor", function(){
		...
		if (jqLinkedEditor.hasClass("codeEditor")) {
			// COMPATIBILITY, support for old code editor
			...
			var codeMirrorHolder = jqDocWrapper.closest("[data-wrapper='documentor']").find(".classDocEditorContainer");
			var CodeMirrorInstance = codeMirrorHolder.data("CodeMirrorInstance");
			var source = CodeMirrorInstance.getDoc().getValue();
