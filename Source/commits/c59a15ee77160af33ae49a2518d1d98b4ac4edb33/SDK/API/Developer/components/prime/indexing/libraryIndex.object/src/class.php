 ...
 * @revised	January 9, 2014, 19:19 (EET)
 */
class libraryIndex
{
	/**
	...
	public static function createMapIndex($path, $domain, $libName)
	{
		...
		catch (Exception $ex)
		{
			...
			$parser->append($root);
			fileManager::create(systemRoot."/".$libPath, "", TRUE);
			$parser->save(systemRoot."/".$libPath, "", TRUE);
		}
	...
	public static function createReleaseIndex($folder, $libName)
	{
		...
		$parser->attr($root, "name", $libName);
		$parser->append($root);
		fileManager::create(systemRoot."/".$libPath."/".self::INDEX_FILE, "");
		return $parser->save(systemRoot."/".$libPath, self::INDEX_FILE, TRUE);
	}
