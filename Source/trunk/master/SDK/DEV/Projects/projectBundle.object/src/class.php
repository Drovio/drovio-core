 ...
 * 
 * @version	3.0-1
 * @created	February 27, 2015, 13:22 (GMT)
 * @updated	October 26, 2015, 10:54 (GMT)
 */
class projectBundle
{
	/**
	...
	{
		// Get current bundle projects
		$bundleProjects = $this->getProjects();
		
		// Create lists to add and remove projects
		$addList = array();
		$removeList = $bundleProjects;
		foreach ($projects as $projectID)
			if (isset($bundleProjects[$projectID]))
				unset($removeList[$projectID]);
			else
				$addList[$projectID] = $projectID;
		
		// Remove projects
		foreach ($removeList as $projectID => $projectInfo)
			$this->removeProject($projectID);
		
		// Add new projects
		foreach ($addList as $projectID => $projectID)
	...
	/**
	 * Remove a project from the bundle.
	 * 
	 * @param	integer	$projectID
	 * 		The project id to be removed.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	...
	{
		// Remove project from bundle
		$dbc = new dbConnection();
		$dbq = new dbQuery("16188462393496", "developer.projects.bundles");
	...
	public function info()
	{
		...
		$dbc = new dbConnection();
		$dbq = new dbQuery("18092509621534", "developer.projects.bundles");
	...
	public function update($title, $description, $tags = "", $price = 0)
	{
		...
		$attr['tags'] = $tags;
		$attr['price'] = (empty($price) ? 0 : $price);
		$attr['id'] = $this->id;
		return $dbc->execute($dbq, $attr);
	}
	...
	public function getProjects()
	{
		...
		if (empty(self::$projects[$this->id]))
		{
			...
			$attr['id'] = $this->id;
			$result = $dbc->execute($dbq, $attr);
			while ($projectInfo = $dbc->fetch($result))
				self::$projects[$this->id][$projectInfo['id']] = $projectInfo;
		}
