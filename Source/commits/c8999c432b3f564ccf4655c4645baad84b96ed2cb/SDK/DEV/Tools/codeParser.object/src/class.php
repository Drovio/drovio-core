 ...
 * @updated	September 18, 2015, 19:46 (EEST)
 */
class codeParser
{
	/**
	...
	public static function clear($code)
	{
		...
		$code = str_replace("&Pi;", "Π", $code);
		$code = str_replace(chr(160), "", $code);
		$code = str_replace("Π", "&Pi;", $code);
		$code = self::decode($code);
	...
	{
		// Decode and trim
		$code = trim(html_entity_decode($code, $flags = ENT_COMPAT | ENT_HTML5, $encoding = 'UTF-8'));
