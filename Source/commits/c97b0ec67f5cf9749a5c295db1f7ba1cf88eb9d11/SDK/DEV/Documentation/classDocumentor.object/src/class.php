 ...
 * 
 * @version	0.1-4
 * @created	July 1, 2014, 22:15 (EEST)
 * @updated	January 1, 2015, 21:59 (EET)
 */
class classDocumentor
{
	...
	public function getInfo()
	{
		...
		$info['daterevised'] = $this->parser->evaluate("//class/info/daterevised")->item(0)->nodeValue; // DEPRECATED
		$info['dateupdated'] = $this->parser->evaluate("//class/info/dateupdated")->item(0)->nodeValue;
		$info['version'] = $this->parser->evaluate("//class/info/version")->item(0)->nodeValue;
		$info['build'] = $this->parser->evaluate("//class/info/build")->item(0)->nodeValue;
		...
		$exceptions = $this->parser->evaluate("//class/info/throws/exception");//->item(0)->nodeValue;
		$info['throws'] = "";
	...
	public function getMethodArray($meth)
	{
		...
		$method['static'] = ($this->parser->attr($meth, "static") || $this->parser->attr($meth, "static") == "true");
	...
	public static function getClassDetails($manual)
	{
		...
		$bld = $parser->evaluate("build", $classInfo)->item(0);
		$details['build'] = $parser->nodeValue($bld);
		...
		// 
		$du = $parser->evaluate("dateupdated", $classInfo)->item(0);
		$details['dateupdated'] = $parser->nodeValue($dr);
	...
	public static function getModel()
	{
		...
		$xmlParser->load(paths::getSDKRsrcPath()."/documentation/classDocumentorModel.xml");
	...
	public function update($objectName, $content = '')
	{
		...
		// DEPRECATED - Set Date Revised
		$dateRevised = $this->parser->evaluate("class/info/daterevised", $root)->item(0);
		$this->parser->nodeValue($dateRevised, $now);
		
		// Set Date Updated
		$dateUpdated = $this->parser->evaluate("class/info/dateupdated", $root)->item(0);
		$this->parser->nodeValue($dateUpdated, $now);
