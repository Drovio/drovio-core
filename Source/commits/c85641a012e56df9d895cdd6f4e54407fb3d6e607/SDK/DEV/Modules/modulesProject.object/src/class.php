 ...
 * @updated	February 20, 2015, 10:24 (EET)
 */
	...
	private function publishSource($version, $branchName = vcs::MASTER_BRANCH)
	{
		...
		while ($row = $dbc->fetch($result))
		{
			...
			foreach ($moduleViews as $viewID => $viewName)
			{
				...
				$contents = BootLoader::resolveURLs(self::PROJECT_ID, $contents, $version, $protocol = NULL, $innerFolder);
				/*$parser = new DOMParser();
			...
			$moduleCSS = BootLoader::resolveURLs(self::PROJECT_ID, $moduleCSS, $version, $protocol = "https", $innerFolder);
			$moduleCSS = cssParser::format($moduleCSS, $compact = TRUE);
			$moduleCSS = BootLoader::resolveURLs(self::PROJECT_ID, $moduleCSS, $version, $protocol = NULL, $innerFolder);
			...
			$moduleJS = BootLoader::resolveURLs(self::PROJECT_ID, $moduleJS, $version, $protocol = NULL, $innerFolder);
