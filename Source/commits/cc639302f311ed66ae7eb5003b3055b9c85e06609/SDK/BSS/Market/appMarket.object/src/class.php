 ...
 * 
 * @version	3.0-5
 * @created	April 3, 2015, 8:42 (BST)
 * @updated	December 16, 2015, 18:14 (GMT)
 */
class appMarket
{
	/**
	...
	public static function getTeamApplications()
	{
		...
		while ($appInfo = $dbc->fetch($result))
		{
			...
			$appInfo['teamName'] = $teamInfo['name'];
			
			// Add to list
			$teamApplications[$appInfo['application_id']] = $appInfo;
		}
	...
	public static function getTeamAppVersion($applicationID, $live = FALSE)
	{
		...
		if (!$live && !empty($appVersion) && !empty($accountValue) && $appVersion['version'] != $accountValue)
			return $accountValue;
	...
	private static function getAccountAppVersion($applicationID)
	{
		...
		$serviceFolder = account::getInstance()->getServicesFolder("/BossApps/");
	...
	private static function setAccountAppVersion($applicationID, $version)
	{
		...
		// Get dashboard service folder
		$accountFolder = account::getInstance()->getServicesFolder("/BossApps/");
		$teamID = team::getTeamID();
