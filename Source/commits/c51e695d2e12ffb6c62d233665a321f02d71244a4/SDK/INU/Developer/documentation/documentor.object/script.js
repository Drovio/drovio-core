...
*/
	...
	*/
	jq(document).on('click', '.ddMenuRoot', function(ev) {
		...
		jq('#'+ref).toggleClass('noDisplay');
		
		var position = jq(this).parent().offset(); 
		jq('#'+ref).css(position);
		var top = parseInt(jq('#'+ref).css('top'));
		jq('#'+ref).css('top', top + height  + 'px');
	});
	...
	*/
	...
	function handleNewSection(html) {
		...
		var key = jq('.documentor > input').clone();
		
		jqxml.find('form > .formBody').append(key);
		var editor = jqxml.html();
		
		jq(".documentor > .dropPool").append(editor);
