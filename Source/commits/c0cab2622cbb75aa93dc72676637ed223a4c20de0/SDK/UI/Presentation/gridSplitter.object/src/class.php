...
{	
	...
	public function build($orientation = self::ORIENT_HOZ, $layout = self::SIDE_RIGHT, $closed = FALSE, $sideTitle = "")
	{
		...
		$selectedOrientation = ($orientation == self::ORIENT_HOZ ? "horizontal" : "vertical" );
		...
		if($orientation == self::ORIENT_HOZ )
		{
			$expPos = 0;
			$expPos = ($layout == self::SIDE_RIGHT ? ($expPos + 1)  : ($expPos + 2) );
		}
		else
		{
			$expPos = 1;
			$expPos = ($layout == self::SIDE_TOP ? ($expPos + 2)  : ($expPos + 0) );
		}
		...
		// Slider
		$slider = DOM::create("div", "", "", ($orientation != self::ORIENT_HOZ ? "hSlider" : "vSlider"));
