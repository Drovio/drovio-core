 ...
 * @revised	May 22, 2014, 22:28 (EEST)
 */
class fileExplorer extends UIObjectPrototype
{
	 ...
	 * 		Define if fileExplorer will be editable (rename, move, copy etc.)
	 * 
	 * @return	fileExplorer
	...
	public function previewFile($fileName, $subPath = "")
	{
		try
		{
			...
			if (is_null($element)){
				$wrap = DOM::create("div");
				DOM::innerHTML($wrap, literal::get("global.notifications.messages.debug", "dbg.content_not_found", NULL, FALSE));
				return $wrap;
			}
			else if($element === FALSE){
				return literal::get("sdk.INU.Views", "msg_previewNotFound");
			}
			else
				return $element;
		} catch (Exception $e)
	...
	public static function getInvalidRoot()
	{
		...
		$invalidTile = self::getStateTile(literal::get("sdk.INU.Views", "msg_invalidRoot"), "invalid_root");
	 ...
	 */
	public function getFileType($filename, $subPath = "")
	{
		// Get extension
		$path = systemRoot.$this->rootPath."/".$subPath."/".$filename;
