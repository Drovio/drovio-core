 ...
 * 
 * @version	0.1-5
 * @created	August 25, 2014, 9:50 (BST)
 * @updated	November 28, 2015, 18:39 (GMT)
 */
class viewTester extends tester
{
	 ...
	 */
	public static function status($appID, $viewName = "all")
	{
		if (empty($appID))
			return FALSE;
		...
		if (empty($viewName) || $viewName == "all")
			return ($status == "all");
		...
		// Return if exists
		return (in_array($viewName, $viewsList) || $status == "all");
	}
