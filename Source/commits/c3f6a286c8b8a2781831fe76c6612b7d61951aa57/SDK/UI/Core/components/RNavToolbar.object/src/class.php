 ...
 * @updated	January 16, 2015, 22:11 (EET)
 */
class RNavToolbar extends UIObjectPrototype
{
	...
	private function buildUserNavigation()
	{
		...
		if (!$loggedIn)
			$item = $toolbarItem->build(literal::dictionary("guest"), "", "userStarter guest", "global")->get();
		else
		{
			...
			if (team::validate())
			{
				$extra = DOM::create("span", "(".team::getTeamName().")", "", "team");
				$extraClass = "withteam";
			}
			$item = $toolbarItem->build($fullName, "", "userStarter ".$extraClass, toolbarItem::SCOPE_GLOBAL, $ico = TRUE, $extra)->get();
	...
	private function buildProfileTools()
	{
		...
		if (privileges::accountToGroup("DEVELOPER") || privileges::accountToGroup("PROJECT_ADMIN") || privileges::accountToGroup("PROJECT_DEVELOPER"))
		{
	...
	private function buildRegionNav()
	{
		...
		$src = $publishFolder."/media/geo/flags/".$localeInfo['imageName'];
		$src = str_replace(paths::getPublishedPath(), "", $src);
		$srcUrl = url::resolve("lib", $src);
	...
	{		
		...
		try
		{
			$parser->load("/System/Resources/SDK/subdomains/navigation/".$this->domain.".xml", true);
		}
		catch (Exception $ex)
		{
			return;
