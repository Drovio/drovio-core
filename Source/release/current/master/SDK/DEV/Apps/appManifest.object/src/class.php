 ...
 * @updated	February 26, 2015, 17:19 (EET)
 */
class appManifest
{
	/**
	...
	public function setPermissions($permissions)
	{
		...
		$parser = new DOMParser();
		$parser->load($indexFile);
		$pRoot = $parser->evaluate("/manifest/permissions")->item(0);
		if (empty($pRoot))
		{
			$root = $parser->evaluate("/manifest")->item(0);
			$pRoot  = $parser->create("permissions");
			$parser->append($root, $pRoot);
		}
		$parser->innerHTML($pRoot, "");
		...
		foreach ($permissions as $prm)
		{
			...
			$parser->append($pRoot, $prmElement);
		}
