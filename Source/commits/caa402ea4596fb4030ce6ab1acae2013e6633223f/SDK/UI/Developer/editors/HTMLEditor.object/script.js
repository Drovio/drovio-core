...
jq(document).one("ready.extra", function(){
	...
	(function(jq){
		...
		jq.fn.htmlEditor = function(/*method*/) {
		...
		function htmlEditor() {
			...
			htmlWrapper.on({
				...
				"drop" : function(ev) {
					...
					/*if (verticalOffset < 10 || horizontalOffset < 10)
						jqtarget.removeClass("dragover").prepend(jq("<"+type+"></"+type+">"));
					else
						jqtarget.removeClass("dragover").append(jq("<"+type+"></"+type+">"));*/
			...
			/*htmlWrapper.on("focusout", ".htmlSection", function(){
			...
			/*htmlWrapper.on("keypress", ".htmlSection", function(ev){
				var p = jq("<p></p>");
				jq(this).contents().filter(function(){
					return this.nodeType == 3;
				}).wrap(p);
