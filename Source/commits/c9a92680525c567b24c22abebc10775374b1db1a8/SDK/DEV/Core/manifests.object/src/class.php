 ...
 * @updated	July 3, 2015, 13:26 (EEST)
 */
class manifests
{
	/**
	...
	public function info($mfID)
	{
		...
		$manifestInfo['info']['retail'] = $parser->attr($manifest, "retail");
		
		// Get manifest title and description
		$manifestInfo['info']['title'] = literal::get("sdk.manifest", "mf_".$manifestInfo['info']['name']."_title");
		$manifestInfo['info']['description'] = literal::get("sdk.manifest", "mf_".$manifestInfo['info']['name']."_desc");
	...
	public function create($name, $packages = array())
	{
		...
		// Set plans
		$this->setPlans($mfID, $premium = FALSE, $retail = FALSE);
	...
	public function setEnabled($mfID, $enabled = TRUE)
	{
		...
		$parser->attr($manifest, "enabled", ($enabled ? 1 : NULL));
	...
	public function setPrivate($mfID, $private = FALSE)
	{
		...
		$parser->attr($manifest, "private", ($private ? 1 : NULL));
	 ...
	 * 		Retail plan permission.
	 * 		Default value is FALSE.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public function setPlans($mfID, $premium = FALSE, $retail = FALSE)
	{
		// Initialize parser
		$parser = $this->xmlParser;
		...
		// Set manifest premium and retail permissions
		$parser->attr($manifest, "premium", ($premium ? 1 : NULL));
		$parser->attr($manifest, "retail", ($retail ? 1 : NULL));
