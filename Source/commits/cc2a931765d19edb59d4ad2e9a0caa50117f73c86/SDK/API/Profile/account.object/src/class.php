 ...
 * @updated	April 14, 2015, 10:43 (EEST)
 */
class account
{
	/**
	...
	private static function getSalt()
	{
		...
		// Check for session
		$sessionSalt = session::get("salt", NULL, "account");
		if (!empty($sessionSalt))
		{
			self::$salt = $sessionSalt;
			return self::$salt;
		}
		...
		// Update Session Salt
		if (empty(self::$salt))
			session::remove("salt", "account");
		else
			session::set("salt", self::$salt, "account");
		return self::$salt;
	}
