...
ascop = ACProtocol = {
	...
	loadingCounter : 0,
	init : function() {
		// Handles preventDefault action on every weblink with href="" or has handler onclick
		jq(document).on("click", "a[href=''], a[onclick]", function(ev) {
			ev.preventDefault();
		});
	...
	request : function(ajaxUrl, method, requestData, sender, successCallback, ajaxOptions) {
		...
		if (jq.type(requestData) == "object") {
			requestData.append("__ASCOP[REQUEST_PATH]", url.getPathname());
			requestData.append("__ASCOP[REQUEST_SUB]", url.getSubdomain());
		} else {
			requestData += "&__ASCOP[REQUEST_PATH]=" + url.getPathname();
			requestData += "&__ASCOP[REQUEST_SUB]=" + url.getSubdomain();
		}
		...
		if (ajaxTester.status()) {
			...
			var paramName = ajaxTester.getParameterName();
			if (jq.type(requestData) == "object")
				requestData.append(paramName, ajaxUrl);
			else
				requestData += "&" + paramName + "=" + ajaxUrl;
			
			ajaxUrl = url.resource("/ajax/tester.php");
		}
		
		// Create request id
		var ascopID = "acp_request_"+Math.random()+"_"+(new Date()).getTime();
		// Make request
		var request = jq.ajax({
			url: ajaxUrl,
			type: method,
			...
			if (typeof successCallback == 'function') {
				successCallback.call(sender, data, textStatus, jqXHR);
			}
		}).fail(function(jqXHR, textStatus, errorThrown) {
			// Log the error report
			logger.dirxml("An error occurred in ascop request. Error:", errorThrown);

			// run errorCallback function, if any
			if (typeof options.errorCallback == 'function') {
				options.errorCallback.call(sender, jqXHR, textStatus, errorThrown);
			}
			...
			if (typeof options.completeCallback == 'function') {
				options.completeCallback.call(sender, jqXHR, textStatus, errorThrown);
			}
