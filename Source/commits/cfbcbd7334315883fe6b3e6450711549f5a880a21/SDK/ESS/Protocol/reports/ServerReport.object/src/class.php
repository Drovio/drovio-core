 ...
 * 
 * @version	2.0-1
 * @created	July 29, 2014, 20:28 (BST)
 * @updated	November 29, 2015, 17:56 (GMT)
 */
abstract class ServerReport extends HTTPResponse
{
	/**
	 ...
	 * @return	void
	 */
	public static function addHeader($header, $key = "", $merge = TRUE)
	{
		if (empty($key))
			self::$headers[] = $header;
		else if ($merge && !empty(self::$headers[$key]))
			self::$headers[$key] = array_merge(self::$headers[$key], $header);
		else
	 ...
	 * @return	void
	 */
	public static function setResponseHeaders($type = self::CONTENT_TEXT_HTML)
	{
		// Initialize Response
		parent::initialize();
		...
		// Set Allow Origin
		$urlInfo = url::info($_SERVER['HTTP_REFERER']);
		...
		parent::setAllowCredentials(TRUE);
	}
}
//#section_end#
?>