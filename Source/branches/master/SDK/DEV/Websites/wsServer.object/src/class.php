 ...
 * 
 * @version	9.0-1
 * @created	October 3, 2014, 19:09 (BST)
 * @updated	December 11, 2015, 14:14 (GMT)
 */
class wsServer
{
	/**
	 ...
	 * 		True on success, false on failure.
	 */
	public function update($name, $address, $connectionType = self::CON_TYPE_FTP)
	{
		$parser = $this->xmlParser;
		...
		// Check if server with the same name already exists
		$server = $parser->evaluate("//server[@name='".$name."']")->item(0);
		if ($serverInfo['name'] != $name && !empty($server))
			return FALSE;
	 ...
	 * 		True on success, false on failure
	 */
	public function updateInfo($description, $serverType, $connectionType = self::CON_TYPE_FTP, $httpdocs = "httpdocs")
	{
		$parser = $this->xmlParser;
