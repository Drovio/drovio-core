 ...
 * @updated	April 14, 2015, 10:22 (EEST)
 */
abstract class DOMPrototype
{
	...
	public static function attr($elem, $name, $val = "")
	{
		if (empty($elem))
		{
			logger::log("You are trying to get an attribute from an empty element.", logger::WARNING);
			return FALSE;
		}
		...
		if ($name == "id")
		{
			...
			if (!$match)
				logger::log("The id attribute value '$val' is not valid.", logger::WARNING);
		}
	...
	public static function appendAttr($elem, $name, $val)
	{
		...
		if (empty($elem))
		{
			logger::log("You are trying to append an attribute to an empty element or the value is empty.", logger::WARNING);
			return FALSE;
		}
	...
	public static function append($parent, $child = NULL)
	{
		if (empty($parent))
		{
			logger::log("You are trying to append an element to an empty parent.", logger::WARNING);
			return FALSE;
		}
	...
	public static function prepend($parent, $child)
	{
		...
		{
			logger::log("prepend() takes no empty elements.", logger::WARNING);
			return FALSE;
		}
	...
	public static function appendBefore($parent, $before, $child)
	{
		...
		{
			logger::log("appendBefore() takes no empty elements.", logger::WARNING);
			return FALSE;
		}
	...
	public static function replace($old, $new)
	{
		if (empty($old))
		{
			logger::log("You are trying to replace an empty element.", logger::WARNING);
			return FALSE;
		}
	...
	public static function innerHTML($element, $value = NULL, $faultTolerant = TRUE, $convertEncoding = TRUE)
	{
		...
		if (empty($element))
		{
			logger::log("You are trying to get or set inner html to an empty element.", logger::WARNING);
			return FALSE;
		}
