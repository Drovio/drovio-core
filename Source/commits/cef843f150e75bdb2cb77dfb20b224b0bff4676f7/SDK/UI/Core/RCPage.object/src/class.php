 ...
 * @updated	October 22, 2015, 1:01 (BST)
 */
class RCPage extends HTMLPageTemplate
{
	...
	protected function __construct($moduleID, $pageAttributes = array())
	{
		...
		// Get current domain and check if we are on team dashboard
		$subdomain = url::getSubDomain();
		if (empty($subdomain) || $subdomain == "www")
			return;
		
		// Get all subdomains to validate
		...
		if (!account::validate())
			$this->moduleID = 373;
		else if (!empty($teamID) && strtolower($teamInfo['uname']) == $subdomain)
		{
			// Check for efplayer
	...
	private function buildPlatformVersioning()
	{
		...
		$ntf = DOM::create("div", "", "", "platform-ntf-updates");
		$closeButton = DOM::create("div", "", "", "close_button");
		DOM::append($ntf, $closeButton);
		
		$title = literal::get("sdk.UI.page", "platform.updates");
		$hd = DOM::create("div", $title, "", "title");
		DOM::append($ntf, $hd);
		
		$title = literal::get("sdk.UI.page", "platform.updates.refresh");
		$refreshButton = DOM::create("div", $title, "", "refresh_button");
		DOM::append($ntf, $refreshButton);
		
		$ntfContainer = DOM::create("div", $ntf, "", "platform-ntf-container");
		DOM::append($this->getBody(), $ntfContainer);
	}
