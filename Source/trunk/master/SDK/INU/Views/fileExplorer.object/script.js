		...
		function fileExplorer() {
			...
			toolbar.on("click", ".rename:not(.disabledButton)", function(ev) {
				...
				inputDiv.one("focusout", function() {
					...
					var oldValue = dataGridList.replaceCell(thisList, firstRow['__index'], 0, newElem);
					var icon = oldValue.find(".previewWrapper").add(oldValue.filter(".previewWrapper")).first();
					newElem.before(icon);
				...
				var oldValue = dataGridList.replaceCell(dtGridList, firstRow['__index'], 0, inputDiv);
				var icon = oldValue.find(".previewWrapper").add(oldValue.filter(".previewWrapper")).first().clone();
				dtGridList.get(0).getRow(firstRow['__index']).data("oldRenameValue", oldValue);
				inputDiv.before(icon).select().focus();
			});
			// Download
