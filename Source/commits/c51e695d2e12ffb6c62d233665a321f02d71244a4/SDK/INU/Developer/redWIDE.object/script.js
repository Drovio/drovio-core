...
jq(document).one("ready.extra", function() {
	...
	jq(document).on({
		"click" : function(){
			toggleHeaders(jq(this));
		}
	}, ".sideTool.headers");

	// Hide headers on outside click
	jq(document).on({
		"click" : function(ev){
			if (jq(ev.target).closest(".headersInnerWrapper").length == 0) {
				toggleHeaders(jq(this));
			}
		}
	}, ".headersContainer")
	
	function toggleHeaders(jqthis) {
		var headers = jqthis.closest("form").find(".headersContainer").first();
		headers.slideToggle(400);
	}
