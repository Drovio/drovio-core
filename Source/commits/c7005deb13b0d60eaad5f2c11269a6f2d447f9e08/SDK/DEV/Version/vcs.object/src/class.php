...
class vcs
{
	 ...
	 */
	private function getItemInfo($id, $branch = self::MASTER_BRANCH)
	{
		// Get working branch
		if (empty($branch))
	...
	public function release($branchName, $version, $title, $description)
	{
		...
		$packageEntry = $parser->evaluate("//releases/branch[@name='".$branchName."']/package[@id='v".$version."']")->item(0);
		if (is_null($packageEntry))
		{
			$packageEntry = $parser->create("package", "", "v".$version);
			$parser->attr($packageEntry, "version", $version);
			$parser->append($branchEntry, $packageEntry);
			
			$parser->update();
		}
		
		// Get build number
		$versionBuild = $parser->attr($packageEntry, "build");
		...
		$parser->attr($packageEntry, "build", "".$versionBuild);
		...
		$parser->append($packageEntry, $versionEntry);
	 ...
	 * 
	 * @deprecated	Use native php function version_compare().
	 */
	...
	private function updateInfo()
	{
		...
		// Update vcs version
		$version = $parser->evaluate("//version")->item(0);
		$parser->nodeValue($version, self::VERSION);
	...
	public function contribute()
	{
		...
		if (empty($author))
		{
			...
			// Update values
			$parser->attr($author, "name", account::getAccountTitle());
			$parser->attr($author, "account", account::getAccountID());
			$parser->update();
		}
		
		// Create working index
		if (file_exists($this->repository.self::VCS_FOLDER.self::WORKING_FOLDER."/".$authorID.".xml"))
			return TRUE;
		...
		return $parser->save($this->repository.self::VCS_FOLDER.self::WORKING_FOLDER, $authorID.".xml");
	}
