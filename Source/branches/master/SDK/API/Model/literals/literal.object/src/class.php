...
class literal extends DEVLiteral
{
	 ...
	 * 		The project's version to get the literals for.
	 * 		It works only in production mode.
	 * 
	 * @return	mixed
	 ...
	 */
	public static function get($projectID, $scope, $name = "", $attributes = array(), $wrapped = TRUE, $locale = NULL, $projectVersion = NULL)
	{
		...
		if (translator::status() || self::onDEV($projectID))
			...
			return self::getLiteral($projectID, $scope, $name, $attributes, $wrapped, $locale, $projectVersion);
	}
	 ...
	 * @return	void
	 */
	private static function getLiteral($projectID, $scope, $name = "", $attributes = array(), $wrapped = TRUE, $locale = NULL, $projectVersion = NULL)
	{
		// Normalize locale
		...
		else
		{
			...
			if (empty($projectVersion))
				$projectVersion = projectLibrary::getLastProjectVersion($projectID, $live = FALSE);
			$publishFolder = projectLibrary::getPublishedPath($projectID, $projectVersion);
