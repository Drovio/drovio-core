 ...
 * @revised	May 10, 2014, 15:01 (EEST)
 */
class vcs
{
	/**
	...
	public function createBranch($branchName, $parent = "")
	{
		...
		$dbq = new dbQuery("18172138902899", "developer.vcs");
		$attr = array();
	...
	public function createItem($id, $path, $name, $smart = FALSE)
	{
		...
		$dbq = new dbQuery("27285167763142", "developer.vcs");
		$attr = array();
		$attr['id'] = $id;
		$attr['pid'] = $this->projectID;
		$attr['path'] = $path;
		...
		//self::$items[$this->projectID][$id] = $attr;
	...
	public function updateItem($id, $forceCommit = FALSE)
	{
		...
		$dbq = new dbQuery("35062959983206", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
	...
	private function getItemInfo($id)
	{
		...
		if (empty(self::$items[$this->projectID][$id]))
		{
			...
			$dbq = new dbQuery("3442352180892", "developer.vcs");
			$attr = array();
			$attr['pid'] = $this->projectID;
			$result = $this->dbc->execute($dbq, $attr);
	...
	public function commit($summary, $description = "", $commitItems = array())
	{
		...
		$dbq = new dbQuery("29497584563464", "developer.vcs");
		$attr = array();
		$attr['id'] = $commitID;
		$attr['pid'] = $this->projectID;
		...
		{
			$dbq = new dbQuery("34372830013116", "developer.vcs");
			$attr = array();
			$attr['cid'] = $commitID;
			$attr['pid'] = $this->projectID;
			$attr['item'] = $itemID;
		...
		foreach ($finalCommitItems as $itemID)
		{
			$dbq = new dbQuery("1434532560983", "developer.vcs");
			$attr = array();
			$attr['pid'] = $this->projectID;
			$attr['item'] = $itemID;
			$attr['branch'] = $workingBranch;
	...
	{
		$dbq = new dbQuery("29795470621483", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
	...
	{
		$dbq = new dbQuery("21149627706753", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $branchName;
		$result = $this->dbc->execute($dbq, $attr);
	...
	public function checkout($branchName)
	{
		...
		$dbq = new dbQuery("23590657828833", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $branchName;
		return $this->dbc->execute($dbq, $attr);
	...
	public function release($branchName, $version, $title, $description)
	{
		...
		$dbq = new dbQuery("22290513142726", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $branchName;
		$result = $this->dbc->execute($dbq, $attr);
		...
		$dbq = new dbQuery("15431180218003", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $branchName;
	...
	public function getHeadBranch()
	{
		...
		$dbq = new dbQuery("33160866393643", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$result = $this->dbc->execute($dbq, $attr);
		$info = $this->dbc->fetch($result);
	...
	public function setWorkingBranch($branchName)
	{
		...
		$dbq = new dbQuery("29533814216138", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
	...
	public function getWorkingBranch()
	{
		...
		if (empty(self::$workingBranch[$this->projectID][$authorID]))
		{
			...
			$dbq = new dbQuery("25354310262439", "developer.vcs");
			$attr = array();
			$attr['pid'] = $this->projectID;
			$attr['author'] = $authorID;
			$result = $this->dbc->execute($dbq, $attr);
	...
	public function getInfo()
	{
		...
		$dbq = new dbQuery("31364061415925", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$result = $this->dbc->execute($dbq, $attr);
	...
	public function getBranches()
	{
		if (empty(self::$branches[$this->projectID]))
		{
			...
			$dbq = new dbQuery("2426181917428", "developer.vcs");
			$attr = array();
			$attr['pid'] = $this->projectID;
			$result = $this->dbc->execute($dbq, $attr);
			...
			foreach ($info as $branchInfo)
				self::$branches[$this->projectID][$branchInfo['name']] = $branchInfo['name'];
		}
	...
	public function getReleases()
	{
		...
		foreach ($branches as $branchName)
		{
			...
			$packages = $parser->evaluate("package", $be);
			foreach ($packages as $package)
			{
				$packageID = $parser->attr($package, "id");
				
				$packageInfo = array();
				$packageInfo['version'] = $parser->attr($package, "version");
				$packageInfo['build'] = $parser->attr($package, "build");
				
				// Branch Releases
				$packageReleases = array();
				$releases = $parser->evaluate("release", $package);
	...
	public function getBranchCommits($branchName = self::MASTER_BRANCH)
	{
		...
		$dbq = new dbQuery("2388701037161", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $branchName;
		$result = $this->dbc->execute($dbq, $attr);
	...
	{
		// Get working items
		$dbq = new dbQuery("22949425783019", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $branchName;
		$attr['item'] = $itemID;
	...
	{
		// Get commit items
		$dbq = new dbQuery("22861257219923", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['cid'] = $commitID;
		$result = $this->dbc->execute($dbq, $attr);
		$itemIDs = $this->dbc->fetch($result, TRUE);
		
		$items = array();
		foreach ($itemIDs as $itemID)
			$items[$itemID] = self::$items[$this->projectID][$itemID];
	...
	public function getWorkingItems()
	{
		...
		// Get working items
		$dbq = new dbQuery("26266178353633", "developer.vcs");
		$attr = array();
		$attr['pid'] = $this->projectID;
		$attr['branch'] = $workingBranch;
		$attr['author'] = $this->getAuthorID();
		...
		foreach ($items as $item)
		{
			...
			$itemInfo = array();
			$itemInfo['path'] = $info['path'].$info['name'];
	...
	/**
	 * Update the vcs old repository to the new repository, database-index-based.
	 * 
	 * @return	{empty}
	 * 		{description}
	 */
	...
	{
		// Create project branches (master only for now)
		$this->createBranch(self::MASTER_BRANCH);
		$workingBranch = $this->getWorkingBranch();
		
		// Get all items and create db entries
		$parser = new DOMParser();
		$parser->load($this->repository.self::VCS_FOLDER."/items/".$workingBranch."/index.xml", FALSE);
		$items = $parser->evaluate("//item");
		foreach ($items as $item)
		{
			// Get item information
			$id = $parser->attr($item, "id");
			$path = $parser->attr($item, "path");
			$name = $parser->attr($item, "name");
			$smart = $parser->attr($item, "folder");
			$smart = (empty($smart) ? 0 : $smart);
		...
		$authors = $parser->evaluate("//author");
		foreach ($authors as $author)
		{
			$id = $parser->attr($author, "id");
			$account = $parser->attr($author, "account");
			$projectAuthors[$id] = $account;
		}
		
		// Get all commits
		$parser->load($this->repository.self::VCS_FOLDER."/commits/index.xml", FALSE);
		$commits = $parser->evaluate("//commit");
		foreach ($commits as $commit)
		{
			// Get commit info
			$commitID = $parser->attr($commit, "id");
			$timestamp = $parser->attr($commit, "time");
			$parent = $parser->attr($commit, "parent");
			$authorID = $parser->attr($commit, "author");
			$author = $projectAuthors[$authorID];
			$summaryItem = $parser->evaluate("summary", $commit)->item(0);
			if (is_null($summaryItem))
			{
				$summary = $parser->nodeValue($commit);
				$description = "";
			}
			else
			{
				$summaryItem = $parser->evaluate("summary", $commit)->item(0);
				$summary = $parser->nodeValue($summaryItem);
				$descItem = $parser->evaluate("description", $commit)->item(0);
				$description = $parser->nodeValue($descItem);
			}
			...
			$parser2->load($this->repository.self::VCS_FOLDER."/commits/".$commitID.".xml", FALSE);
			$commitRoot = $parser2->find($commitID);
			$commitItems = $parser2->evaluate("items/item", $commitRoot);
			foreach ($commitItems as $item)
			{
				// Get item id
				$itemID = $parser2->attr($item, "id");
		...
		{
			// Get release info
			$version = $parser->attr($release, "version");
			$versionBuild = $parser->attr($release, "build");
			$versionTime = $parser->attr($release, "time");
			$title = $parser->attr($release, "title");
			$description = $parser->nodeValue($release);
	...
	public function getAuthors()
	{
		...
		$accounts = $project->getProjectAccounts();
