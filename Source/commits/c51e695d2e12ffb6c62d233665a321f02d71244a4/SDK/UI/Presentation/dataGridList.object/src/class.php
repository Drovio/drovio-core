 ...
 * @revised	April 28, 2013, 14:52 (EEST)
 */
class dataGridList extends UIObjectPrototype
{
	...
	public function __construct()
	{
		...
		$this->hSize = 0;
		$this->vSize = 0;
		$this->textClass = "dataGridTextWrapper";
		$this->itemClass = "dataGridCell";
		$this->checkItemClass = "dataGridCheck";
	...
	/**
	 * Creates and returns the dataGridList container
	 * 
	 * @param	string	$id
	 ...
	 * @return	DOMElement
	 * 
	 * @deprecated	use dataGridList::build() and dataGridList::get() methods instead.
	 */
	public function create_grid($id = "", $checkable = FALSE)
	{
		return $this->build($id, $checkable)->get();
	}
	...
	public function build($id = "", $checkable = FALSE)
	{
		...
		$wrapper_class = "dataGridWrapper";
		$list_class = "dataGridList";
		$checkableClass = ($checkable ? " checkable" : "");
		
		$gridListWrapper = DOM::create("div", "", $this->id, $wrapper_class." ".$this->initClass.$checkableClass);
		$this->set($gridListWrapper);
		
		$this->gridList = DOM::create("ul", "", "", $list_class);
	...
	public function setColumnRatios($ratios)
	{
		...
		$totalRatio = (!$this->checkable ? 100 : 100 - 8);
	 ...
	 * @return	void
	 * 
	 * @deprecated	use dataGridList::setHeaders
	 */
	public function set_headers($header_contents)
	{
		$this->setHeaders($header_contents);
	}
	...
	public function setHeaders($header_contents)
	{
		...
		$wrapper_class = "dataGridContentWrapper";
		$this->gridListContentWrapper = DOM::create("div", "", "", $wrapper_class);
	 ...
	 * @return	mixed
	 * 
	 * @deprecated	use dataGridList::insertRow instead
	 */
	public function insert_row($contents = array(), $checkName = NULL, $checked = FALSE)
	{
		return $this->insertRow($contents, $checkName, $checked);
	}
	...
	private function pr_insert_row($contents, $class = "")
	{
		...
		foreach ($contents as $key => $c)
		{
			$itemIdentifier = "";
			if (gettype($c) == "string") {
				$grid_text = DOM::create("span", $c, "", $this->textClass);
				$itemIdentifier = strtolower(str_replace(" ", "", $c));
			}
			else if ($c->tagName == "span")
			{
				$grid_text = DOM::create("span", "", "", $this->textClass);
			...
			$grid_item = DOM::create("div", "", "", $this->itemClass);
	...
	{
		$check_item = DOM::create("div", "", "", $this->checkItemClass);
		$chk = form::input("checkbox", $name = $checkName);
		$chk = DOM::import($chk);
