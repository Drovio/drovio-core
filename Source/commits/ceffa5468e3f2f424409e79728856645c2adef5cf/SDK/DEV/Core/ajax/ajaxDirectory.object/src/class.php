 ...
 * @updated	April 7, 2015, 11:57 (EEST)
 */
class ajaxDirectory
{
	/**
	 ...
	 */
	public static function create($dirName, $parentDir = "")
	{
		// Library Path
	...
	/**
	 * Remove an ajax directory.
	 * The directory must be empty of pages and other directories.
	 * 
	 * @param	string	$dirName
	 * 		The directory full path, separated by "/".
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	...
	{
		// Library Path
		$libPath = self::updateMapFilepath();
		...
		// Get directory
		$pdir = explode("/", $dirName);
		$expression = "//map/dir[@name='".implode("']/dir[@name='", $pdir)."']";
		$dirElement = $parser->evaluate($expression)->item(0);
		if (empty($dirElement) || $dirElement->childNodes->length > 0)
			return FALSE;
		
		// Remove dir
		$parser->replace($dirElement, NULL);
		return $parser->update();
	}
}
//#section_end#
