 ...
 * Redback JS Document
 *
 * Title: RedBack JS Library - jquery.redPopups | popup library
...
jq(document).one("ready.extra", function() { 
	...
	jq.ajax({
		url: _host+"/ajax/apps/aux_content.php?app=2&fl=ajax::faspp", 
		dataType: "html", 
		// on success
		success: function(div) {
			var jqDiv = jq(div);
			asContainer = jqDiv.children("[data-container='wrapper']");
			asGroup = jqDiv.children("[data-container='group']");
			asListItem = jqDiv.children("[data-container='groupItem']");
			asPopup = jqDiv.children("[data-container='popup']");
		}
	})
	.error(function() { 
	...
	*/
	...
	jq(document).on({
		...
		/*"change.autosuggest" : function(ev) {
			var jqthis = jq(this);
			if (jqthis.val().length>2) {
				jqthis.trigger("updated.autosuggest");
			}
		}, 
		*/
					...
					jq.ajax({
						...
						success: function(contentXML) {
							...
							jq(contentXML).find("group").each(function(index) {
								...
								//var extraGroupItems = asGroupClone.data("asExtraItems")/(asGroupClone.data("asGroups")-index);
								//asGroupClone.data("asExtraItems", asGroupClone.data("asExtraItems") - extraGroupItems);
	...
	/*var div = jq("<div style='width:40px;height:40px;position:absolute;top:200px;left:300px;background-color:black;' />")
				.append(jq("<div style='width:1px;height:100%;position:absolute;border-left:1px solid white;left:50%' />"))
				.append(jq("<div style='width:100%;height:1px;position:absolute;border-top:1px solid white;top:50%' />"));
	div.on("click", function() {
		div.redPopups("ajaxGlobalPopup", 
			{
				"sender": div, 
				"binding":"on", 
				"type":"obedient", 
			...
			{
				sender: div,  
				context: jq("<div style='width:200px;height:100px;background-color:red;' />").append(jq('<div class="uiPpArrow"></div>')).append(jq("<div style='width:1px;height:100%;position:absolute;border-left:1px solid black;left:50%' />")).append(jq("<div style='width:100%;height:1px;position:absolute;border-top:1px solid black;top:50%' />")), 
