 ...
 * Extension View
 * 
 * Represents an extension view object manager.
 * 
 * @version	{empty}
 * @created	May 23, 2014, 10:06 (EEST)
 * @revised	May 23, 2014, 10:06 (EEST)
 */
class extView
{
	/**
	...
	public function create($viewName)
	{
		...
		// Create object index
		$status = $this->ext->addObjectIndex("views", "view", $viewName);
		if (!$status)
			return FALSE;
		
		// Create vcs object
		$itemID = $this->getItemID();
		$itemPath = extension::VIEWS_FOLDER;
		$itemName = $viewName.".".self::FILE_TYPE;
		$viewFolder = $this->vcs->createItem($itemID, $itemPath, $itemName, $smart = TRUE);
	 ...
	 * 		Otherwise it returns only the view code section.
	 * 
	 * @return	string
	 * 		The php source code.
	 */
	...
	{
		$path = systemRoot.paths::getDevRsrcPath()."/headers/extension/view.inc";
		$header = fileManager::get($path);
		$header = phpParser::unwrap($header);
		
		// Add application id
		$header .= "\n\n".phpParser::variable("extID").' = '.$this->ext->getID().";\n";
		
		// Initialize importer
		//$header .= "importer::initApp(".$this->ext->getID().");\n";
