...
tabber = {
	init : function() {
		...
		jq(document).on("add.tab", '.tabControl', function(ev, editable, tab_id, tab_title, tab_context) {
			// Add a new tab
			tabber.addTab(jq(this), tab_id, tab_title, tab_context, editable);
		});
	...
	resize : function(jqTabber) {
		...
		if (jqTabber.hasClass("full")) {
			var totalTabs = jqTabber.find('.tabMenuItem').length;
	...
	addTab : function(jqTabber, tab_id, tab_title, tab_context, editable) {
		// Get Sender
		jqTabber.data("context", tab_context);
		
		// Set async parameters
		var tabber_id = jqTabber.attr("id");
		var tabParameters = "o_id=Presentation.tabControl&o_path=tabber&o_type=content";
		tabParameters += "&editable="+editable+"&holder_id="+tabber_id+"&tab_id="+tab_id+"&header="+tab_title;
		...
		var successCallback = function(data) {
			...
			if (jqTabber.find('#'+tabPage.attr('id')+'.tabPageContainer').length > 0) {
				...
				if (replace) {
					// Fill Tab Page
					jqTabber.data("context").appendTo(tabPage);
		...
		// Get tab content
		ascop.asyncRequest("/ajax/resources/sdk/tabControl/tabber.php", "GET", tabParameters, "html", jqTabber, successCallback, null, true, true);
	}
}