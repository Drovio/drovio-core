 ...
 * @updated	November 4, 2015, 11:31 (GMT)
 */
class ePerson
{
	/**
	...
	/**
	 * Initialize person instance with account id.
	 * 
	 * @param	integer	$accountID
	 * 		The user account id.
	 * 
	 * @return	boolean
	 * 		True if there is a connected relation to the given account id, false otherwise.
	 */
	public function initWithAccount($accountID)
	{
		// Get current team id
		...
		// Get person information by account id
		$rdbc = new rdbConnection();
		$q = new dbQuery("29990822818233", "enterprise.relations.persons");
		$attr = array();
		$attr['tid'] = $teamID;
		$attr['aid'] = $accountID;
	...
	public function import($accountID)
	{
		...
		// Check if there is already a person imported with the given account id
		$alreadyImported = $this->initWithAccount($accountID);
		if ($alreadyImported)
			return TRUE;
