 ...
 * 
 * @version	13.1-3
 * @created	February 18, 2014, 10:04 (GMT)
 * @updated	November 18, 2015, 20:03 (GMT)
 */
class project
{
	/**
	 ...
	 * 
	 * @param	boolean	$publishResources
	 * 		Whether to include resources when publishing.
	 * 		It is TRUE by default.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public function publish($version, $publishResources = TRUE)
	{
		// Validate account to project
		if (!$this->validate())
			return FALSE;
		...
		// Publish project literals
		$this->publishLiterals($publishFolder);
	...
	/**
	 * Publish project's literals to production library folder.
	 * 
	 * @param	string	$publishFolder
	 * 		The project's published folder.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public function publishLiterals($publishFolder)
	{
		// Check if publish folder is valid
		if (empty($publishFolder))
			return FALSE;
			
		// Create literals folder
		$literalsFolder = $publishFolder."/".projectLibrary::LT_FOLDER;
		folderManager::create(systemRoot.$literalsFolder);
		
		// Get all active locale and create their folders
		$activeLocale = locale::active();
		foreach ($activeLocale as $locInfo)
			folderManager::create(systemRoot.$literalsFolder."/".$locInfo['locale']);
		
		// Get all project scopes
		$scopes = literal::getScopes($this->id);
		foreach ($scopes as $scInfo)
			foreach ($activeLocale as $locInfo)
			{
				// Get scope literals in the given locale
				$scope = $scInfo['scope'];
				$locale = $locInfo['locale'];
				$literals = literal::get($this->id, $scope, $name = "", $attributes = array(), $wrapped = FALSE, $locale);
				
				// Create json to export
				$literals_json = json_encode($literals, TRUE);
				fileManager::create(systemRoot.$literalsFolder."/".$locale."/".$scope.".json", $literals_json);
			}
