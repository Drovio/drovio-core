 ...
 * @updated	September 18, 2015, 16:36 (EEST)
 */
class module
{
	/**
	...
	private function loadModuleInfo()
	{
		...
		$this->modulePath = mGroup::getTrail($this->groupID).$this->getDirectoryName($this->id);
	}
	...
	public function updateInfo($title, $description = "")
	{
		...
		$dbq = new dbQuery("33263572205447", "modules");
	...
	public function create($title, $parentID, $description = "")
	{
		...
		$dbc = new dbConnection();
		$dbq = new dbQuery("22631684549188", "modules");
	...
	public function remove()
	{
		...
		$dbc = new dbConnection();
		$q = new dbQuery("27683222049264", "modules");
		
		$attr = array();
		$attr['id'] = $this->id;
		$status = $dbc->execute($q, $attr);
		
		// Remove module's literals
		if ($status)
			literal::removeScope(modulesProject::PROJECT_ID, APIModule::getLiteralScopeName($this->id));
	...
	public function loadCSS()
	{
		...
		foreach ($views as $viewID => $viewName)
		{
			...
			$viewsCSS .= $viewObject->getCSS($normalCss = TRUE)."\n";
		}
