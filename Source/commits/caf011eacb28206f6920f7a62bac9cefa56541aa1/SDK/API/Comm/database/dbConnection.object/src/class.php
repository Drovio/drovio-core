 ...
 * @updated	October 28, 2015, 9:40 (GMT)
 */
class dbConnection
{
	/**
	...
	public function execute($query, $attr = array(), $commit = TRUE)
	{
		...
		{
			// Log activity
			logger::log("Executing SQL Query to [".$this->database."] at ".$this->host, logger::DEBUG, $query);
		...
		catch (Exception $ex)
		{
			...
			logger::log("Query Execution to [".$this->database."] at ".$this->host." failed: ".$this->error, logger::ERROR, $query);
