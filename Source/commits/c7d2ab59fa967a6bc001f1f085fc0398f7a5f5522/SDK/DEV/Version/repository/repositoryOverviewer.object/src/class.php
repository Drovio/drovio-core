 ...
 * @revised	February 19, 2014, 13:25 (EET)
 * 
 * @deprecated	Functionality is moving towards redback modules.
 */
class repositoryOverviewer extends UIObjectPrototype
{
	...
	private function getCommitSection($targetgroup)
	{
		...
		// Create commitContainer for the commit list
		$commitContainer = DOM::create("div", "", "", "commitContainer");
		DOM::append($navContainer, $commitContainer);
		...
		foreach ($branches as $branchName => $branchData)
		{
			...
			$pageCommits = array_slice($commits, 0, 30);
			...
			if (count($commits) > 30)
			{
				...
				DOM::append($navContainer, $pagination);
