	...
	update : function(init) {
		...
		jq(".timestamp.live").each(function() {
			...
			if (timeDiff < 3) {
				...
				// It's seconds
				if (init)
					context = timeDiff + " seconds ago";
				else
					context = "a few seconds ago";
			}
			else if (timeDiff < 60*60) {
				// It's minutes
				var minutes = Math.floor(timeDiff / 60);
				if (minutes == 1)
					context = "about a minute ago";
				else
					context = minutes + " minutes ago";
			}
			else if (timeDiff < 24*60*60) {
				// It's hours
				var hours = Math.floor(timeDiff / (60*60));
				if (hours == 1)
					context = "about an hour ago";
				else
					context = hours + " hours ago";
			}
				...
				if (days == 1)
					context = "yesterday at "+(new Date(utime*1000)).format("H:i");
				else
					context = days + " days ago";
			}
			else
