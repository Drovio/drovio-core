 ...
 * @revised	March 26, 2014, 10:12 (EET)
 */
class BootLoader
{
	/**
	...
	{
		$id = self::getFileName(self::$prefix[$category], $library, $package, "css");
		self::loadCSS($id);
	}
	...
	{
		$id = self::getFileName(self::$prefix[$category], $library, $package, "js");
		self::loadJS($id);
	}
	...
	private static function getURL($url, $library, $package, $tester = FALSE, $type = "")
	{
		...
		else
		{
			...
			//$params['hs'] = self::getFileName(self::$prefix[$library], $library, $package, $type);
		}
