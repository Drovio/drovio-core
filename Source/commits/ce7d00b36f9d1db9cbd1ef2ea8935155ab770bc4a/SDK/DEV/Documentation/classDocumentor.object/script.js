...
jq(document).one("ready.extra", function() {
	...
	// If subdomain is not developer, exit script
	...
	jq(document).off("content.modified.classDocumentor");
	jq(document).on("content.modified.classDocumentor", function() {
		...
		documentorWrappers.each(function() {
			jq(this).removeAttr("data-wrapper").classDocumentor("init");
		});
	});
	...
	(function(jq){
		...
		var methods = {
			init : function(){
				...
				var togglerTmpl = jqDocWrapper.find(".documentorContainer .cld_pool").children();
			
				// need to check this
				var jqExpanderUnit = togglerTmpl.clone(true, true);
				var jqStaticExpanderUnit = jqExpanderUnit;
...
*/						
					...
					jqDocEditor.on("reveal.documentation", function(){
						...
						*/
						...
						classExpander.trigger("open.toggler");
						...
						constantsCollection.not(":empty").each(function(){
							...
							constantsExpander.trigger("setHead.toggler", [jq("<span>Constants</span><span class='expanderCounter'>"+constantsCollection.find(".constantTileWrapper").length+"</span>")]);
						...
						propertiesInfoExpander.trigger("setHead.toggler", [jq("<span>Properties</span><span class='expanderCounter'></span>")]);
						...
						for (var i = 0; i < scopeTags.length; i++) {
							...
							propertiesCollection.not(":empty").each(function(){
								...
								propertiesExpander.trigger("setHead.toggler", [jq("<span>"+scopeTags[i]+"</span><span class='expanderCounter'>"+propertiesCollection.find(".propertyTileWrapper").length+"</span>")]);
						...
						methodsInfoExpander.trigger("setHead.toggler", [jq("<span>Methods</span><span class='expanderCounter'></span>")]);
						...
						for (var i = 0; i < scopeTags.length; i++) {
							...
							methodsCollection.not(":empty").each(function(){
								...
								methodsExpander.trigger("setHead.toggler", [jq("<span>"+scopeTags[i]+"</span><span class='expanderCounter'>"+methodsCollection.find(".methodTileWrapper").length+"</span>")]);
