 ...
 * @updated	October 30, 2015, 16:38 (GMT)
 */
class application
{
	/**
	...
	private static function getApplicationPath()
	{
		...
		$appVersion = self::getApplicationVersion($applicationID);
	 ...
	 * @param	integer	$applicationID
	 * 		The application id.
	 * 		If there is an application currently running, it will be selected.
	 * 
	 * @return	string
	 * 		The application running version.
	 */
	public static function getApplicationVersion($applicationID = "")
	{
		// Get application id
		$runningApplicationID = self::init();
		$runningApplicationID = (empty($runningApplicationID) ? $applicationID : $runningApplicationID);
		if (empty($runningApplicationID))
			return NULL;

		// Get team's or app's running version according to running environment
		if (self::onBOSS())
			$appVersion = appMarket::getTeamAppVersion($runningApplicationID);
		else if (self::onAPC())
			$appVersion = appSessionManager::getInstance()->getVersion($runningApplicationID);
		
		// Get last version if external request
		if (empty($appVersion))
			$appVersion = projectLibrary::getLastProjectVersion($runningApplicationID);
	 ...
	 */
	public static function onEnterprise()
	{
		// Get subdomain where the application is running
		$subdomain = appTester::currentDomain();
	 ...
	 * 		True if the application is running on enterprise dashboard, false otherwise.
	 * 
	 * @deprecated	Use onEnterprise() instead.
	 */
	public static function onBOSS()
	{
		return self::onEnterprise();
	}
