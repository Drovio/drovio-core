...
class invoice
{
	...
	public function close()
	{
		...
		// Get all products to get price
		$iProduct = new invoiceProduct($this->invoiceID);
		$invoiceProducts = $iProduct->getAllProducts();
		$totalPrice = 0;
		$totalTax = 0;
		foreach ($invoiceProducts as $productInfo)
		{
			$totalPriceNoTax = round($productInfo['total_price'] / (1 + $productInfo['tax_rate']), 2);
			$totalPrice += $totalPriceNoTax;
			$totalTax += $productInfo['total_price'] - $totalPriceNoTax;
		}
		...
		$attr['total_tax'] = $totalTax;
		$attr['total_payments'] = $totalPayments;
