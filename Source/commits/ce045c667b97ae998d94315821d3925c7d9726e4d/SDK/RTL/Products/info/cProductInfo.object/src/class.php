 ...
 * @revised	December 18, 2014, 18:08 (EET)
 */
class cProductInfo
{
	/**
	 ...
	 * 
	 * NOTE:
	 * To add a company specific info, you must be team admin.
	...
	public function add($categoryID, $title, $is_bool = FALSE, $generic = FALSE)
	{
		...
		if (!$generic && !accountKey::validateGroup("TEAM_ADMIN", company::getCompanyID(), accountKey::TEAM_KEY_TYPE))
			return FALSE;
	 ...
	 * 
	 * NOTE:
	 * To set a company specific info value, you must be team admin.
	...
	public function set($infoID, $value = NULL)
	{
		...
		// Check if account is in the right user group
		if ($this->productType == self::COMPANY_PRODUCT && !accountKey::validateGroup("TEAM_ADMIN", company::getCompanyID(), accountKey::TEAM_KEY_TYPE))
			return FALSE;
