 ...
 * @updated	September 3, 2015, 12:53 (EEST)
 */
class sdkObject extends classObject
{
	...
	{  
		$path = systemRoot.paths::getDevRsrcPath()."/core/headers/sdkObject.inc";
	...
	/**
	 * Get the object's manual context.
	 * 
	 * @param	string	$locale
	 ...
	 * 
	 * @return	string
	 * 		The object's manual context.
	 */
	public function getManual($locale = NULL)
	{
		// Get Item trunk folder
		$itemFolder = $this->getObjectFullPath();
		
		// Get manual contents (COMPATIBILITY)
		if (file_exists($itemFolder."/manual.html"))
			return fileManager::get($itemFolder."/manual.html");
		
		// Get manual web document
		$manDoc = new sdkObjectManual($itemFolder, "", "manual");
		return $manDoc->get($locale);
	}
	
	/**
	 * Update the object's manual.
	 * 
	 * @param	string	$content
	 * 		The manual content.
	 * 
	 ...
	 * 		True on success, false on failure.
	 */
	public function updateManual($content = "", $locale = NULL)
	{
		// Get Item trunk folder
		$itemFolder = $this->getObjectFullPath();
		...
		// COMPATIBILITY (Remove old file)
		fileManager::remove($itemFolder."/manual.html");
		
		// Set manual web document
		$manDoc = new sdkObjectManual($itemFolder, "", "manual");
		return $manDoc->update($content, $locale);
	}
