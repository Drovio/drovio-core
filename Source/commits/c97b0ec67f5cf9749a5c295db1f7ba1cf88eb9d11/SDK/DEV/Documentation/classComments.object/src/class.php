...
class classComments
{
	...
	private function getClassStampComments($library, $package, $namespace)
	{
		...
		$classStamp['copyright'] = 'Copyright (C) '.date('Y').' Redback. All rights reserved.';
	...
	private function getClassInfoComments($title, $description, $version, $created, $updated, $throws = "", $deprecated = "")
	{
		...
		$classInfo['updated'] = trim($updated);
		if (is_numeric($classInfo['updated']))
			$classInfo['updated'] = intval($classInfo['updated']);
		$classInfo['updated'] = (empty($classInfo['updated']) ? date("F j, Y, G:i (T)") : date("F j, Y, G:i (T)", $classInfo['updated']));
