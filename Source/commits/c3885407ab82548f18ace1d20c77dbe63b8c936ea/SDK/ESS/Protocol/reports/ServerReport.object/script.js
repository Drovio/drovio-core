...
ServerReport = {
	request : function(serverUrl, jqSender, method, requestData, successCallback, errorCallback, completeCallback, loading, callType) {
		...
		var reportSuccessCallback = function(report) {
			ServerReport.handleReport(jqSender, report, callType, successCallback);
		}
	...
	handleReport : function (sender, report, callType, parseContentCallback) {
		...
		BootLoader.loadResources(report, function() {
			...
			if (typeof parseContentCallback == 'function')
				parseContentCallback.call(sender, report, callType);
			
			// Parse report for actions
			ServerReport.parseReportActions(sender, report, callType);
		});
	...
	parseReportActions : function(sender, report, callType) {
		// Load body context
