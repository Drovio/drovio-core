 ...
 * @updated	December 16, 2015, 23:45 (GMT)
 */
class account
{
	/**
	...
	public static function getInstance($teamName, $authToken = "")
	{
		...
		if (!isset(self::$instances[$teamName]))
			self::$instances[$teamName] = new account($teamName, $authToken);
	...
	/**
	 * Set the authentication token from an external source.
	 * 
	 * @param	string	$authToken
	 * 		The current authentication for the account instance.
	 * 
	...
	public function validate($logoutOnFail = FALSE)
	{
		...
		$payload = authToken::getPayload($this->authToken);
