...
FormProtocol = {
	...
	submit : function(ev, jqForm) {
		...
		var handlerFunction = function(report) {
			FormProtocol.handleReport(jqForm, report);
		...
		if (jqForm.data("form-action") != undefined) {
			...
			ascop.asyncRequest(formAction, "POST", formInputs, "html", jqForm, bootLoaderCallback, null, false);
		}
	},
	...
	handleReport : function (jqForm, report) {
		// Get form actions
		jq(report).find("action").each(function() {
			switch (jq(this).data("attr").value) {
				case "submit":
					jqForm.removeData("preventUnload");
					break;
				case "reset":
					jqForm.trigger('reset');
					break;
