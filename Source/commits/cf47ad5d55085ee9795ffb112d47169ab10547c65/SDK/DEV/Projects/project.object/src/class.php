 ...
 * @updated	April 3, 2015, 10:25 (EEST)
 */
class project
{
	/**
	...
	public function info()
	{
		...
		$projectIcon = $this->getResourcesFolder()."/.assets/icon.png";
		if (file_exists(systemRoot.$projectIcon))
		{
			// Resolve path
			$projectIconUrl = str_replace(paths::getRepositoryPath(), "", $projectIcon);
			$projectInfo['icon_url'] = url::resolve("repo", $projectIconUrl);
		}
	 ...
	 * 
	 * @param	boolean	$open
	 * 		Indicates an open source project.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public function updateInfo($title, $description = "", $open = FALSE)
	{
		// Load project info
		$this->loadInfo();
	 ...
	 * The project release must be in status 1, under review. If the release goes to published or rejected status, it cannot be removed.
	 * 
	 * @param	string	$version
