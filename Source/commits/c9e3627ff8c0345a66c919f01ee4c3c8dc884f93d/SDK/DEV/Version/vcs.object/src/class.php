...
class vcs
{
	...
	public function getReleases()
	{
		...
		foreach ($branches as $branchName => $branchData)
		{
			...
			$branchInfo['current'] = $branchData["base_version"];
			
			// Get release packages
			$dbq = new dbQuery("14677962695274", "developer.vcs");
			$attr = array();
			$attr['pid'] = $this->projectID;
			$attr['branch'] = $branchName;
			$result = $this->dbc->execute($dbq, $attr);
			$packages = $dbc->fetch($result, TRUE);
			...
			foreach ($packages as $package)
			{
				...
				$dbq = new dbQuery("1647945435111", "developer.vcs");
				$attr = array();
				$attr['pid'] = $this->projectID;
				$attr['branch'] = $branchName;
				$attr['version'] = $package["version"];
				$result = $this->dbc->execute($dbq, $attr);
				$releases = $dbc->fetch($result, TRUE);
				
				$packageReleases = array();
				foreach ($releases as $release)
				{
					$releaseVersionID = "v".$release['version'];
