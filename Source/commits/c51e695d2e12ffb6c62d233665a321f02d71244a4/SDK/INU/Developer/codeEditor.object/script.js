	...
	var methods = {
		init : function(lang) {
			...
			return this.each(function(){
				...
				// instance variables
				codeEditorHabitat = jq(".codeEditor_habitat", this);
			...
			//Embed functionality
				...
				/*scriptWrapper.on({
					"focusout": function(ev){
						if(jq.type(userSelectionRange) != "null" && jq.type(userSelectionRange) != "undefined"){
							var highlightRange = document.createRange();
							highlightRange = userSelectionRange.cloneRange();
							highlightRange.surroundContents(jqhighlight.get(0));
						}
					},
					
					"focusin": function(ev){
							jq(this).find(".highlight").contents().unwrap();
				...
				/*presentationArea.off("click.lineMap", ".presentationArea > .lineNumber");
				presentationArea.on("click.lineMap", ".presentationArea > .lineNumber", function(ev){
					if (!ev.ctrlKey)
						return;
					
					var jqthis = jq(this);
					var idx = presentationArea.find(".lineNumber").index(jqthis);
					jqthis.toggleClass("mark");
					if (jqthis.hasClass("mark")) {
						var last = scriptArea.children().last().index();
						var top = (idx/last)*100;
						var title = jq.trim(scriptArea.children(".code").eq(jq(ev.target).index()/2).text());
						if (title.length > 100)
							title = jq.trim(title.substring(0, 100))+"..."
						else
							title = jq.trim(title.substring(0, 100));
						codeEditorHabitat.find(".lineMap").prepend(jq("<div title='["+(idx+1)+"] "+title+"' />").addClass("mark").data("lineIndex", idx).css("top", top+"%"));
					} else {
						codeEditorHabitat.find(".lineMap div").filter(function() {
							return jq(this).data("lineIndex") == idx;
						}).remove();
					}
					
					var nodeToSelect = scriptArea.children(".active");
					...
					userSelectionRange = document.createRange();
					userSelectionRange.selectNodeContents(nodeToSelect.get(0));
					userSelectionRange.collapse(false);
					var userSelection = getUserSelection();
					userSelection.removeAllRanges();
					userSelection.addRange(userSelectionRange);
				});*/
				
				// Line selection Handlers (needs check!)
				/*var mouseDown = false;
				var codeDown = jq();
				presentationArea
					.off("mousedown.codeEditor", ".presentationArea > .lineNumber")
					.off("mouseenter.codeEditor", ".presentationArea > .lineNumber");
				jq(document).off("mouseup.codeEditor");
				presentationArea.on("mousedown.codeEditor", ".presentationArea > .lineNumber", function(ev){
					mouseDown = true;
					var jqthis = jq(this);
					var nodeToSelect = scriptArea.children(".code").removeClass("active").eq(jq(ev.target).index()/2);
					codeDown = nodeToSelect;
					//declare active
					nodeToSelect.addClass("active");
				}).on("mouseenter.codeEditor", ".presentationArea > .lineNumber", function(){
					if (mouseDown){
						var jqthis = jq(this).next();
						codeDown.removeClass("active").addClass("contentAltered");
						jqthis.addClass("active");
						var startNode = (jqthis.index() <= codeDown.index() ? jqthis : codeDown);
						var endNode = (jqthis.index() > codeDown.index() ? jqthis : codeDown)
						
						//select nodes
						var tmpRange = document.createRange();
						tmpRange.selectNodeContents(endNode.get(0));
						tmpRange.setStart(startNode.get(0), 0);
						var userSelection = getUserSelection();
						userSelection.removeAllRanges();
						userSelection.addRange(tmpRange);
					}
				});
				jq(document).on("mouseup.codeEditor", function(){
					mouseDown = false;
					codeDown = jq();
				});*/
				...
				scriptArea.on("paste.manager.codeEditor",function(ev){
					...
					jqthis.doTimeout("pasteTimeout", 1, function() {
						...
						/*jqthis.children(".code").children().not("br").each(function(){
						});*/
						/*if(jqthis.children(".active").children().not("br").length != 0){
							//click
							console.log(jqthis.children(".active").clone().get(0));
							jqthis.children(".active").contents().not(jqthis.children(".active").children()).remove();
							jqthis.children().removeClass("contentAltered")
									.filter(".active")
										.children().removeClass("active")
													.addClass("contentAltered")
													.attr("style", "")
										.last().addClass("active").end()
										.unwrap();
						}else{*/
						// ?
...
//						var sContainer = jqthis.children(".active").eq(0).contents().get(0);
				...
				/*scriptArea.off("focusout.inactive.codeEditor").off("focusin.inactive.codeEditor");
				scriptArea.on({
					"focusout.inactive.codeEditor" : function(ev) {
						scriptWrapper.addClass("inactive");
					},
					
					"focusin.active.codeEditor" : function(ev) {
						scriptWrapper.removeClass("inactive");
					},
				});*/
...
//					console.log(new Date().getTime() - start);
					...
					if ((57>=ev.which && ev.which>=48) //number
						...
						if (ev.which == 13) {
							...
							jqthis.doTimeout("typeTimeout", 1, /*setTimeout(*/function() {							
								// fix for empty texts (!)
...
//console.log(new Date().getTime() - start);
						...
						//jqthis.doTimeout("preProcessTimeout", true);
						/*jqthis.doTimeout("preProcessTimeout", 50, */
...
//console.log(new Date().getTime() - start);
					...
					// _____
//console.log("Width restored [iteration end]");
//console.log(new Date().getTime() - start);
...
/*if (jq.type(middle) != "undefined"){
	console.log("Part presented");
}else{
	console.log("Presented");
}
console.log(new Date().getTime() - start);*/
				...
				jq(this).on("changeFontSize.codeEditor", function(ev, offsetWithOperator) {
					var size = jq(".codeEditor_habitat > .scriptWrapper > .typingWrapper > .scriptArea", this)
						.css("min-height", offsetWithOperator).css("min-height");
					jq(".codeEditor_habitat > .scriptWrapper", this)
						.css("line-height", (offsetWithOperator == "" ? "" : size))
						.css("font-size", (offsetWithOperator == "" ? "" : size));
				});
				
				//jq(document).off("click.lineMap", ".lineMap .lineMapToolbar .activeLine");
				jq(document).on("click.lineMap", ".lineMap .lineMapToolbar .activeLine", function() {
					scriptArea.find(".active").first().scrollHere();
				});
				
				//jq(document).off("click.lineMap", ".lineMap .lineMapToolbar .lineMapButton:not(.activeLine)");
				...
				//jq(this).off("initializeLineMap.lineMap");
				jq(this).on("initializeLineMap.lineMap", function(ev, callback) {
					var byFlow = lineMap.find(".byFlow");
					scriptArea.children(".code").filter(":contains('function ')").each(function(){
					...
					scriptArea.children(".code").filter(":contains('.on'), :contains('.one')").each(function(){
					...
					if (lang == "css") {
						scriptArea.children(".code").filter(":contains('{')").each(function(){
				...
				scriptArea.on("mouseup.selection.codeEditor", function(){
							...
							/*.on("mouseup.activePrompt.codeEditor", function(){
								placePrompt(userSelectionArray, jqprompt);
							})*/
							.on("mouseup.activeLine.codeEditor", function(ev){
								defineActiveLine(userSelectionArray, scriptArea);
							})
							/*.on("mouseup.activeWord.codeEditor", function(){
								currentWord = defineActiveWord(userSelectionArray);
							})*/
							/*.on("mouseup.activePopup.codeEditor", function(){
								popUse(currentWord, jqprompt, lang);
							})*/
							...
							/*.on("keydown.activePrompt.codeEditor", function(){
								setTimeout(function(){
									placePrompt(userSelectionArray, jqprompt);
								}, 1);
							})*/
							...
							/*.on("keydown.activeWord.codeEditor", function(){
								setTimeout(function(){
									currentWord = defineActiveWord(userSelectionArray);
								}, 1);
							})*/
							/*.on("keydown.activePopup.codeEditor", function(){
								setTimeout(function(){
									popUse(currentWord, jqprompt, lang);
								}, 1);
							})*/
...
//global functions
	...
	function popUse(currentWord, prmpt, lang){
		...
		popupTimer = setTimeout(function(){
			...
			if (currentWord.match(new RegExp(expression))){
			}
				/*prmpt.redPopups("ajaxGlobalPopup", 
					{
						"sender": prmpt, 
						"binding":"on", 
						"type":"obedient", 
					...
					{
						sender: prmpt, 
						context: jq("<div>"+currentWord+"</div>"), 
						position: 'right|top'
					}
				);*/
		}, 400);
...
jq(document).one("ready.extra",function() {
	...
	jq(document).on("content.modified", function() {
		var WIDEs = jq(".codeEditor").filter("[data-type]");
		
		if (WIDEs.length > 0 && jq.fn.codeEditor.loadedResources !== true)
		...
		WIDEs.each(function() {
			...
			jq(this).codeEditor(type)
					.find(".scriptArea")
						.trigger("init.content.codeEditor")
						.end()
