...
jq(document).one("ready", function(){
	...
	jq(document).on("content.modified.wViewEditor", function(){
		//if (!wasInitiated)
			//return;
			
		jq(".wViewEditor").filter(function(){
		...
		}).removeAttr("data-css-property").removeAttr("data-default-value").end().wViewEditor();
...
(function(jq){
	...
	var methods = {
		init : function(){			
			return this.each(function(){
				wViewEditorInitialize.call(this);
			});		
		}
	}
	
	jq.fn.wViewEditor = function() {
	...
	function wViewEditorInitialize(){
		// Initialize variables
		var thisPseudoWrapper = pseudoWrapper.clone();
		var wViewEditor = jq(this);
		var structureWrapper = wViewEditor.find(".structureWrapper");
		var structureViewer = structureWrapper.children('.structureViewer');
		var previewWrapper = wViewEditor.find(".previewWrapper");
		var modelEditor = wViewEditor.find(".modelEditor");
		wViewEditor.data("hasModel", modelEditor.length > 0);
		var cssContainer = wViewEditor.find(".cssContainer");		
		// Initialize template
		...
		var structure = jq("<div>").html(wViewEditor.find("[name='structure']").detach().val());
		var jqpath = structureWrapper.children(".path");
		if (!wViewEditor.data("hasModel"))
		...
		wViewEditor.on("initialize.wViewEditor", function(ev, cssCoder){
		// This needs timeout to let the code script load... we need to do something for this...
			...
			jq(cssCoder).trigger("getCodeEditorContent", [true, function(content){
				...
				renderPreview(wViewEditor, content, initialCollection);
			}]);
		});
		
		// Initialize Preview
		wViewEditor.trigger("initialize.wViewEditor", cssCode.get(0));
		
		// Respective editor's content has changed
		cssContainer.on("text.modified.wViewEditor", function(){
			jq(this).data("code-modified", "true");
		});
		wViewEditor.on("text.modified.wViewEditor", ".modelEditor", function(){
			jq(this).data("code-modified", "true");
		});
		var t1 = null;
		cssContainer.on("keydown.wViewEditor, focusin.wViewEditor", ".currentSelectorPath", function(ev) {
			var jqthis = jq(this);
			...
			t1 = setTimeout(function(){
				...
				wViewEditor.trigger("path.changed.wViewEditor");
		...
		wViewEditor.on("path.changed.wViewEditor", function(ev){
		...
		cssContainer.on("focusout.wViewEditor", ".propertiesView input, .propertiesView select", function(){
			jq(this).closest(".propertyWrapper").removeClass("focused");
		});
		
		cssContainer.on("focusin.wViewEditor", ".propertiesView input, .propertiesView select", function(){
			jq(this).closest(".propertyWrapper").addClass("focused");
			jq(this).data("previousCSSValue", jq(this).val());
		});
		// Selection Input
		cssContainer.on("change.wViewEditor", ".propertiesView .selectionInput > select", function(){
			jq(this).siblings("input").val(jq(this).val()).change();
		});
		cssContainer.on("change.wViewEditor", ".propertiesView .selectionInput > input", function(){
			jq(this).siblings("select").val("");
		});
		// Selection Input
		cssContainer.on("change.wViewEditor", ".propertiesView .timeInput > select, .propertiesView .lengthInput > select", function(){
			jq(this).siblings("input").change();
		});
		// Update Criterion
		cssContainer.on("change.wViewEditor", ".propertiesView .propertyWrapper input, .propertiesView .propertyWrapper > select", function(){	
			...
			if (val != jq(this).data("previousCSSValue"))
				wViewEditor.trigger("update.styles.wViewEditor", [true, function(){
					cssCode.trigger("getCodeEditorContent", [true, function(c){
						renderPreview(wViewEditor, c, initialCollection);
					}]);
				}]);
		});
	
		// Update the css styles
		wViewEditor.on("update.styles.wViewEditor", function(ev, updateEverything, callback){
			var txt = "";
		...
		wViewEditor.on("keyup.wViewEditor", ".cssContainer .cssCurrentSelector", function(ev){
		...
		structureViewer.on("initializeStructure.wViewEditor", function(ev){
			...
			jq(".previewWrapper", wViewEditor).html(structure.clone().children());
		});
		
		structureViewer.trigger("initializeStructure.wViewEditor");
		
		// Toggle views in CSS structure, while parsing new css from the editor, if any
		wViewEditor.on("click.wViewEditor", ".cssContainer .cssTool.toggleCoder", function(ev){
		...
		wViewEditor.on("click.wViewEditor", ".cssContainer .cssTool.restore", function(){
			...
			wViewEditor.trigger("update.styles.wViewEditor", true);
		...
		wViewEditor.on("change.wViewEditor", ".previewBar .selectBrowser > select", function() {
			try {
		...
		jq(document).off("click.wViewEditor.pseudowrapper");
		jq(document).on("click.wViewEditor.pseudowrapper", function(ev){
			if (!jq(ev.target).is(jq(".wViewEditor .structureWrapper > .path > .element")))
				jq(".pseudoWrapper").css("display", "");
		});
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .path > .element", function(){
		...
		// Edit element
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .path > .element > .edit", function(ev){
		...
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .path .equality", function() {
		...
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .pseudoWrapper .pseudoClass, .structureWrapper > .pseudoWrapper .pseudoElement", function(){
	...
	*/
		...
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .path > .pseudoElement, .structureWrapper > .path > .pseudoClass, .structureWrapper > .path > .relation", function(ev){
		...
		wViewEditor.on("focusout.wViewEditor", ".structureWrapper > .path .editableArea", function(ev){
			wViewEditor.trigger("path.changed.wViewEditor");
		});
		
		// Hover above a pseudoelement's editable area in the path
		wViewEditor.on("mouseenter.wViewEditor", ".structureWrapper > .path > .pseudoClass > .editableArea",function(){
			jq(this).parent(".pseudoClass, .pseudoElement").css("background-color","transparent");
		})
		.on("mouseleave.wViewEditor", ".structureWrapper > .path > .pseudoClass > .editableArea", function(){
			jq(this).parent(".pseudoClass, .pseudoElement").css("background-color","");
		});
		
		// Handle hover class in structure viewer
		wViewEditor.on('mouseenter.wViewEditor', '.structureWrapper > .structureViewer *:not([class="elementClass"], [class="elementTag"], [class="elementId"], [class="elementAttr"])', function(ev){
			structureViewer.find('*').removeClass("hover");
			jq(this).addClass("hover");
		});
		wViewEditor.on('mouseleave.wViewEditor', '.structureWrapper > .structureViewer *', function(ev){
		        jq(this).removeClass("hover");
			jq(ev.relatedTarget).closest('.wViewEditor .structureWrapper > .structureViewer *:not(.elementClass, .elementTag, .elementId, .elementAttr)').addClass("hover");
		});
	    	
		// Click a clickable element in the viewer
	    	wViewEditor.on("click.wViewEditor", ".structureWrapper > .structureViewer .elementTag, .structureWrapper > .structureViewer .elementClass, .structureWrapper > .structureViewer .elementId, .structureWrapper > .structureViewer .elementAttr", function(){
	        	var jqthis = jq(this);
		...
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .structureViewer", function(ev){
			if (ev.ctrlKey)
				clearStructureSelection.apply(structureViewer);
		});
		/*
		wViewEditor.on("click.wViewEditor", ".cssContainer #wViewEditor_formView", function(ev){
			if (!ev.ctrlKey)
				return;
		...
		wViewEditor.on("mouseenter.wViewEditor", ".structureWrapper > .path", function() {
			jq("<div title='Add multiple selectors' class='plus' />").appendTo(jq(this));
		});
		wViewEditor.on("mouseleave.wViewEditor", ".structureWrapper > .path", function(){
			jq(this).children(".plus").remove();
		});
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .path > .plus", function(){
		...
		wViewEditor.on("click.wViewEditor", ".structureWrapper > .pathSibling", function(){
			jq(this).remove();
			wViewEditor.trigger("path.changed.wViewEditor");
		});
	
		// Toggle views in the preview area
		wViewEditor.on("click.wViewEditor", ".previewBar .showHTMLPage", function(ev){
		...
		// Toggle views in the preview area
		wViewEditor.on("click.wViewEditor", ".previewBar .showHTMLEditor", function(ev){
			var jqbar = jq(this).closest(".previewBar");
		...
		// Search / Store css value for a tag
		wViewEditor.on("keyup.wViewEditor", ".cssContainer .searchCss", function(ev){
			var val = jq(this).val();
...
jq(document).on("click.wViewEditor", ".wViewEditor .viewOptions .horizontalSplit, .wViewEditor .viewOptions .verticalSplit", function(){
			var jqsplit = jq(this);
			//var splits = jqsplit.closest(".wViewEditor").data("splits");
			...
			//jqsplit.closest(".wViewEditor").data("splits", ++splits);
	...
	function renderPreview(wViewEditor, code, initialCollection) {
		if (!wViewEditor.data("hasModel"))
			return;
	
		// Get style identifier
		var cesi = wViewEditor.attr("data-cesi");
		...
		if (jq.trim(cesi) == "") {
			...
			cesi = "ce_"+r;
			wViewEditor.attr("data-cesi", cesi);
		}
		
		// Get all wViewEditor styles from head
		var styles = jq(document.head).find("style").filter(function(){
			return jq(this).data("origin", "wViewEditor");
		});
		
		// Remove styles that are not used anymore
		styles.filter(function(){
			var editor = jq(".wViewEditor[data-cesi='"+jq(this).data("cesi")+"']", document.body);
		...
		for (var selector in initialCollection) {
			var proper = selector.replace(/[^,]+/gm, function(mat){
				return " .wViewEditor[data-cesi='"+cesi+"'] .previewContainer > .previewWrapper "+mat;
			});
		...
		// Create style
		styleElement = jq("<style type='text/css'></style>").data("origin", "wViewEditor").data("cesi", cesi).text(code);
	...
	function setPreviewStyles(browser) {
		...
		jq(element).data("origin", "wViewEditor");
	}
...
function handlePropertiesInfo(xml) {
	...
	wasInitiated = true;
	jq(document).trigger("content.modified.wViewEditor");
}