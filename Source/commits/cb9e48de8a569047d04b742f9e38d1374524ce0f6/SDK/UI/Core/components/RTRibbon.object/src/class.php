 ...
 * @revised	November 6, 2014, 13:32 (EET)
 */
class RTRibbon extends UIObjectPrototype
{
	 ...
	 */
	public function addCollection($id, $title = "", $moduleID = "", $viewName = "", $startup = FALSE)
	{
		// Ribbon Collection
		$collection = $this->createCollection($id, $title, $moduleID , $viewName, $startup);
	 ...
	 * 		The collection element.
	 */
	public static function createCollection($id, $title = "", $moduleID = "", $viewName = "", $startup = FALSE)
	{
		// Ribbon Collection
		$collection = DOM::create("div", "", $id, "collection noDisplay");
		DOM::attr($collection, "data-title", $title);
		if (!empty($moduleID))
