...
class application
{
	...
	/**
	 * Get all application styles from the library.
	 * 
	 * @param	integer	$applicationID
	 * 		The application id.
	 * 
	 ...
	 * 
	 * @return	array
	 * 		An array with all application library styles.
	 */
	public static function getAppLibraryStyles($applicationID, $applicationVersion = "")
	{
		// Get last project version
		if (empty($applicationVersion))
			$applicationVersion = projectLibrary::getLastProjectVersion($applicationID);
		
		return self::getAppLibraryObjects($applicationID, $applicationVersion, $objectName = "style");
	}
	
	/**
	 * Get all application scripts from the library.
	 * 
	 * @param	integer	$applicationID
	 * 		The application id.
	 * 
	 ...
	 * 
	 * @return	array
	 * 		An array with all application library scripts.
	 */
	public static function getAppLibraryScripts($applicationID, $applicationVersion = "")
	{
		// Get last project version
		if (empty($applicationVersion))
			$applicationVersion = projectLibrary::getLastProjectVersion($applicationID);
		
		return self::getAppLibraryObjects($applicationID, $applicationVersion, $objectName = "script");
	}
	
	/**
	 * Get all library objects with the given name (tag name).
	 * 
	 * @param	integer	$applicationID
	 * 		The application id.
	 * 
	 ...
	 * 
	 * @return	array
	 * 		An array of all application library objects.
	 */
	private static function getAppLibraryObjects($applicationID, $applicationVersion, $objectName)
	{
		// Check application id
		if (empty($applicationID))
			return NULL;
		...
		else
		{
			...
			try
			{
				$parser->load($applicationPath."/".DEVApplication::INDEX_FILE);
			}
			catch (Exception $ex)
			{
				return NULL;
			...
			// Get styles
			$libStyles = array();
			$indexElements = $parser->evaluate("//".$objectName);
			foreach ($indexElements as $element)
				$libStyles[] = $parser->attr($element, "name");
