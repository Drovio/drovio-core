 ...
 * @revised	August 14, 2014, 12:21 (EEST)
 */
class RCPage extends HTMLPageTemplate
{
	...
	private function setStaticResources($resources)
	{
		foreach ($resources as $rsrc)
		{
			...
			if ($rsrc['css'])
			{
				...
				$this->addStyle($rsrcCssHref, $rsrc['id'], TRUE);
			}
			...
			if ($rsrc['js'])
			{
				...
				$this->addScript($rsrcJsHref, FALSE, $rsrc['id'], TRUE);
			}
		}
	}
	
	/**
	 * Adds a css style to the document, connected as a BootLoader resource.
	 * 
	 * @param	string	$href
	 * 		The href attribute of the stylesheet.
	 * 
	 * @param	string	$rsrcID
	 * 		The resource id.
	 * 
	 ...
	 * 		The css element.
	 */
	protected function addStyle($href, $rsrcID = "")
	{
		$css = parent::addStyle($href);
		DOM::attr($css, "data-id", $rsrcID);
		DOM::attr($css, "data-static", TRUE);
		
		return $css;
	}
	
	/**
	 * Adds a js script to the document, connected as a BootLoader resource.
	 * 
	 * @param	string	$src
	 ...
	 * 		The script element.
	 */
	protected function addScript($src, $bottom = FALSE, $rsrcID = "")
	{
		$script = parent::addScript($src, $bottom);
		DOM::attr($script, "data-id", "static:".$rsrcID);
