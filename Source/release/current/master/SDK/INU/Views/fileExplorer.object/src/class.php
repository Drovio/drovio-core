 ...
 * 
 * @version	4.0-4
 * @created	April 26, 2013, 14:02 (BST)
 * @updated	October 25, 2015, 18:10 (GMT)
 */
class fileExplorer extends UIObjectPrototype
{
	...
	{
		// Set and check root path
		$this->rootPath = $rootPath."/";
		if (trim($rootPath, "/ \t\n\r\0\x0B") == "" || (!is_dir(systemRoot.$this->rootPath) && !is_dir($this->rootPath)))
			$this->rootPath = NULL;
		
		// Set file explorer id
		$this->id = (empty($id) ? "fxp".time().mt_rand() : $id);
		
		// Set root friendly name
		try
		{
			$this->rootFriendlyName = (is_string($friendlyRootName) ? $friendlyRootName : DOM::innerHTML($friendlyRootName));
		}
		catch (Exception $ex)
		{
			$this->rootFriendlyName = "";
		}
	 ...
	 * 
	 * @param	array	$maskItems
	 * 		An array of mask items.
	 * 		This array can be used to change folder names to be readable by users.
	 * 		Mask the array's key with the value.
	 * 
	 * @return	fileExplorer
	 * 		The fileExplorer object
	 */
	public function build($subPath = "", $maskItems = array())
	{
		// Create fileExplorer object holder
		$wrapper = DOM::create("div", "", $this->id, "fileExplorer");
		$this->set($wrapper);
		
		// Set attributes
		DOM::data($wrapper, "path", basename($this->rootPath));
		if ($this->readOnly)
			DOM::data($wrapper, "readonly", TRUE);
		
		// Check root path and break process if doesn't exist
		...
		session::set($this->id."_mask", $maskItems, "fileExplorer");
		...
		$fileAreaHolder = DOM::create("div", "", $this->id."_flv", "fileViewer");
		...
		// If file explorer is ready only, hide preferences
		if ($this->readOnly)
			return $this;
		...
		$view = DOM::create("div", "", $this->id."_fldv", "folderView");
	...
	/**
	 * Get the mask items array for the given file explorer identifier.
	 * 
	 * @param	string	$rootIdentifier
	 * 		The file explorer identifier.
	 * 
	 * @return	array
	 * 		The mask items array.
	 */
	public static function getMaskItems($rootIdentifier)
	{
		return session::get($rootIdentifier."_mask", $default = array(), $namespace = 'fileExplorer');
	}
	 ...
	 * 
	 * @param	integer	$mode
	 * 		The file creation unix mode.
	 * 		Default value is 0777.
	 * 
	 * @param	boolean	$recursive
	 * 		Whether to perform recursive creation.
	 * 		It is FALSE by default.
	 * 
	 * @return	boolean
	 * 		The status of creation
	 */
	public function createFolder($folderName, $subPath = "", $mode = 0777, $recursive = FALSE)
	{
		// Check root path and read only
		if (empty($this->rootPath) || $this->readOnly)
			return FALSE;
		
		// Create folder
		return folderManager::create(systemRoot.$this->rootPath."/".$subPath."/", $folderName, $mode, $recursive);
	}
	...
	public function renameFile($path, $oldName, $newName)
	{
		...
		// Check that is not executable file
		if ($this->isExecutable($newName))
			return FALSE;
	...
	/**
	 * Move an uploaded file to a fileExplore inner folder.
	 * 
	 * @param	string	$uploadedFilePath
	 * 		The uploaded file path.
	 * 
	 * @param	string	$filePath
	 * 		The new file path, relative to the file explorer root path.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public function moveUploadedFile($uploadedFilePath, $filePath)
	{
		// Validate instance
		if (empty($this->rootPath) || $this->readOnly)
			return FALSE;
		
		// Validate that is not executable file
		if ($this->isExecutable($filePath))
			$filePath = $filePath."_";
		
		// Move uploaded file
		return move_uploaded_file($uploadedFilePath, systemRoot.$this->rootPath."/".$filePath);
	}
	
	/**
	 * Checks whether a given filename is executable based on the extension.
	 * It includes all major web extensions like php, aspx, jsp etc.
	 * 
	...
	{
		$toolAreaWrapper = DOM::create("div", "", "", "fx-toolbar");
	...
	{
		$pathWrapper = DOM::create("div", "", "", "fx-pathbar");
	...
	{
		$uploadArea = DOM::create("div", "", $this->id."_ua", "fx-uploadArea");
	...
	public function getFileType($filename, $subPath = "", $extensionOnly = FALSE)
	{
		...
		// If request only extension, return extension
		if ($extensionOnly)
			return $ext;
	 ...
	 */
	public static function isIconified($extension)
	{
