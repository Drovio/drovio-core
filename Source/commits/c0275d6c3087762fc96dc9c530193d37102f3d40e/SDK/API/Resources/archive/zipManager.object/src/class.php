 ...
 * @revised	July 28, 2014, 16:12 (EEST)
 */
class zipManager
{
	/**
	...
	{	
		foreach ((array)$contents['dirs'] as $dir)
		{
			...
			{
				logger::log("Failed to create inner directory ".$innerDirectory.$localname.basename($dir).". Aborting...", logger::WARNING);
				$zip->unchangeAll();
				return FALSE;
			}
		...
		foreach ((array)$contents['files'] as $file)
		{
			if ($zip->addFile($file, $innerDirectory.$localname.basename($file)))
				continue;
			logger::log("Failed to pack ".$innerDirectory.$localname.basename($file).". Aborting...", logger::WARNING);
			$zip->unchangeAll();
			return FALSE;
		}
	...
	private static function removeFiles($zip, $contents)
	{
		foreach ($contents as $file)
		{
			...
			{
				logger::log("Invalid entry identifier. Remove Incomplete.", logger::WARNING);
				$zip->unchangeAll();
				return FALSE;
			}
			...
			foreach ((array)$children as $details)
			{
				if ($zip->deleteName($details['name']))
					continue;
				logger::log("Could not delete ".$details['name'].". Aborting...", logger::WARNING);
				$zip->unchangeAll();
				return FALSE;
			}
		}
	...
	private static function renameFiles($zip, $contents, $newNames)
	{
		...
		foreach ($contents as $key => $file)
		{
			if (!is_string($file) && !is_int($file))
			{
				logger::log("Invalid entry identifier. Rename Incomplete.", logger::WARNING);
				$zip->unchangeAll();
				return FALSE;
			}
			...
			foreach ((array)$children as $details)
			{
				if ($zip->renameName($details['name'], preg_replace('/'.preg_quote($file, '/').'/', $newNames[$key], $details['name'], 1)))
					continue;
				logger::log("Could not rename ".$details['name'].". Aborting...", logger::WARNING);
				$zip->unchangeAll();
				return FALSE;
			}
		}
	...
	private static function copyFiles($zip, $origins, $destinations)
	{
		...
		foreach ($origins as $key => $file)
		{
			if (!is_string($file) && !is_int($file))
			{
				logger::log("Invalid entry identifier. Copy Incomplete.", logger::WARNING);
				$zip->unchangeAll();
				return FALSE;
			}
