 ...
 * @updated	May 13, 2015, 14:07 (EEST)
 */
class registerDialog extends UIObjectPrototype
{
	...
	private function buildMainForm($regtype = self::REG_TYPE_PAGE, $return_url = "")
	{
		...
		// Left side (logos and other)
		$leftSide = DOM::create("div", "", "", "leftSide side");
		DOM::append($mainContainer, $leftSide);
		
		$title = literal::get("sdk.UI.register", "lbl_regTitle");
		$htitle = DOM::create("h3", $title, "", "title");
		DOM::append($leftSide, $htitle);
		
		$title = literal::get("sdk.UI.register", "lbl_regSub");
		$hsub = DOM::create("h4", $title, "", "sub");
		DOM::append($leftSide, $hsub);
		
		// Services container
		$srvContainer = DOM::create("div", "", "", "services");
		DOM::append($leftSide, $srvContainer);
		
		// Redback Logos
		$logo = DOM::create("div", "", "", "logo global");
		DOM::append($srvContainer, $logo);
		
		$logo = DOM::create("div", "", "", "logo biz");
		DOM::append($srvContainer, $logo);
		
		$logo = DOM::create("div", "", "", "logo web");
		DOM::append($srvContainer, $logo);
		
		$logo = DOM::create("div", "", "", "logo apps");
		DOM::append($srvContainer, $logo);
		
		// Form container
		$formContainer = DOM::create("div", "", "", "formContainer side");
		DOM::append($mainContainer, $formContainer);
		...
		// Firstname
		$input = $form->getInput($type = "text", $name = "firstname", $value = "", $class = "uiRegInput small f", $autofocus = TRUE, $required = TRUE);
		...
		// Lastname
		$input = $form->getInput($type = "text", $name = "lastname", $value = "", $class = "uiRegInput small", $autofocus = FALSE, $required = TRUE);
