...
jq(document).one("ready.extra", function() {
		...
		gLists.each(function(){
			...
			this.getRow = function(ri) {
				...
				if (jq.type(ri) == "number") {
					row = jq(this).find(".uiDataGridList .dataGridRow").eq(ri);
				} else {
					row = jq(this).find(".uiDataGridList .dataGridRow").filter(function(){
	...
	// sort on click
	jq(document).on("click", ".gridList > .dataGridHeader > .dataGridCell", function(){
		jqthiscol = jq(this).closest(".gridList")
							.find(".dataGridRow")
	...
	jq(document).on("addRow", ".uiDataGridList", function(ev, contents){
		...
		var ratio = 100;
		if (jq(this).closest(".uiDataGridList").hasClass("checkable")) {
			ratio = 100 - 8;
