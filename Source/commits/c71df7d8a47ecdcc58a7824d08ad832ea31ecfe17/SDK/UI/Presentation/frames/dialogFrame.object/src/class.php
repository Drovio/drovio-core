 ...
 * @updated	July 1, 2015, 13:08 (EEST)
 */
class dialogFrame extends windowFrame
{
	...
	{
		// Build frame
		$windowFrame = parent::build($title, "dialogFrame", $background)->get();
	...
	private function buildControls($type = self::TYPE_OK_CANCEL)
	{
		...
		$this->formFactory->appendControl($controlsContainer);
	 ...
	 * 
	 * @param	boolean	$requiredFormNotes
	 * 		If set to true and there are required input fields, a note for required fields will be inserted in the form.
	 * 		It is TRUE by default.
	 * 
	 * @return	string
	 * 		The server's frame report.
	 */
	public function getFrame($requiredFormNotes = TRUE)
	{
		// Append form to frame
		parent::append($this->formFactory->get($requiredFormNotes));
