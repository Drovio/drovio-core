 ...
 * CSS Editor
 * 
 * A web environment for css manipulation
 * 
 * @version	{empty}
 * @created	April 26, 2013, 14:13 (EEST)
 * @revised	April 26, 2013, 14:13 (EEST)
 */
class cssEditor extends UIObjectPrototype
{
	...
	private function getPreviewContainer($browsers)
	{
		...
		*/		
		...
		$toggleHTMLEditor = DOM::create("div", "", "", "toggleHTMLEditor previewTool");
		DOM::attr($toggleHTMLEditor, "data-toggle-name", "HTML");
		$toggleHTMLText = DOM::create("span", "Page");
		DOM::append($toggleHTMLEditor, $toggleHTMLText);
		
		DOM::append($previewBar, $toggleHTMLEditor);
	...
	private function getCssContainer($measurement, $tabs, $tiles)
	{
		...
		$cssToolbar = new toolbar();
		$cssToolbarElement = $cssToolbar->build($dock = "T");
		...
		$cssToolbar->insert_tool($cssPreviousCollection);
		...
		*/
		...
		foreach ($tabs as $gid => $tab)
		{
			...
			// Insert Tab Page
			$cssTabber->insert_tab($gid, $tab, $page, $selected);
			$selected = FALSE;
		}
	...
	private function getCssCoder($measurement, $tabs, $tiles)
	{
		...
		$cssToolbar->insert_tool($cssSelector);
