 ...
 * @revised	August 28, 2014, 17:04 (EEST)
 */
class application
{
	/**
	...
	public static function import($package, $class = "")
	{
		...
		if (appTester::publisherLock())
		{
			...
			try
			{
				return importer::req($applicationPath."/".devApplication::SOURCE_FOLDER."/".$package."/".$namespace."/".$class.".php", $root = TRUE, $once = TRUE);
			}
			catch (Exception $ex)
			{
		...
		try
		{
			...
			if (appTester::publisherLock())
			{
				...
				try
				{
					...
					return importer::req($applicationPath."/".devApplication::VIEWS_FOLDER."/".$viewName.".php", $root = TRUE, $once = TRUE);
				}
				catch (Exception $ex)
				{
					// Catches the exception thrown by the programmer's code
			...
			else
			{
				...
				if (viewTester::status(self::$applicationID, $viewName))
	...
	{
		// Get team's runnng application version
		$teamVersion = projectLibrary::getTeamProjectVersion(self::$applicationID);
		if (empty($teamVersion))
			return NULL;
		
		// Get version token published path
		return projectLibrary::getPublishedPath(self::$applicationID, $teamVersion);
	}
