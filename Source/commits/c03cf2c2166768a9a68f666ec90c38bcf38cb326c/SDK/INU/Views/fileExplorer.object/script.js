...
jq(document).one("ready.extra", function(){
	...
	(function(jq){
		...
		function fileExplorer() {
			...
			fileExplorer.on("listUpdated.dataGridList", function(ev, selected){
				if (selected == 0)
					toolbar.trigger("init");
				else
			...
			toolbar.on("click", "[name='newFolder']", function(ev) {
				if (fileViewer.find(".dataGridList").length == 0)
					return;
				...
				inputDiv.one("focusout", function() {
					...
					if (folderName == "") {
						var thisList = jq(this).closest(".dataGridList");
					...
					var rowIdentifier = jq(this).closest(".dataGridList").trigger("replaceCell", [0, 0, cont]).get(0).assignRowIdentifier(0);
				...
				fileViewer.find(".stateViewer").filter(function(){
					...
					.find(".dataGridList").trigger("addRow", [container, "", "folder", ""]);
			});
			// Rename
			toolbar.on("click", "[name='rename']", function(ev) {
				var dtGridList = fileViewer.find(".dataGridList");
			...
			toolbar.on("click", "[name='download']", function(ev) {
				var glist = fileViewer.find(".dataGridList");
				if (glist.length == 0)
					return;
		...
		function uploadFiles(files, subpath) {
			...
			for (var f = 0; f < files.length; f++) {
				...
				var sameFileNameIndex = jq(this).find(".dataGridList").get(0).searchRows("name", new RegExp(files[f].name));
				...
				var gList = jq(this).find(".dataGridList");
				...
				var ajaxUrl = url.resource("/ajax/resources/sdk/fileExplorer/putFiles.php");
				jq.ajax({
					url: ajaxUrl,
					type: "POST",
					data: fd,
					...
					success: function(resp, status, xhr){
						...
						if (jq.type(resp) == "undefined" || jq.type(resp) == "null" || resp.status !== true) {
							...
							completedBar.addClass("invalid").each(function(){
								...
								var row = jqthis.closest(".dataGridList").get(0).getRow(ri);
								...
								setTimeout(function(){
									// Row Identifier
									jqfv.find(".dataGridList").trigger("removeRow", ri);
								}, 3500);
							});
						}
						else {
							...
							completedBar.each(function(){
								...
								jqthis.animate({
									...
									var gList = jqfv.find(".dataGridList");
		...
		function folderCheck(response, folderName, rowIdentifier) {
			...
			if (response.status !== true && jq.type(rowIdentifier) == "string"){
				...
				fviewer.find(".dataGridList").trigger("removeRow", rowIdentifier)
			}else {
				...
				var gList = fviewer.find(".dataGridList");
		...
		function dropCheck(response, rows) {
			...
			if (jq.type(statuses) == "string") {
				...
				// Reappend rows
				for (var row in rows) {
					fviewer.find(".dataGridList").trigger("addRow", [icon.clone().filter("span").text(rows[row].name).end(), rows[row].size, rows[row].type, rows[row].modified]);
		...
		function downloadFiles(subpath, fileNames) {
			...
			jq('#fediframe').attr('src', "/ajax/resources/sdk/fileExplorer/downloadFiles.php?"+data);
