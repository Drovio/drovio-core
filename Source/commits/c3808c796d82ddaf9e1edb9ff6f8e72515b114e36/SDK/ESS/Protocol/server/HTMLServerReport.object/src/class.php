 ...
 * @revised	December 20, 2013, 11:56 (EET)
 */
class HTMLServerReport extends JSONServerReport
{
	 ...
	 * 
	 * @param	{type}	$forceHolder
	 * 		{description}
	 * 
	 * @return	ServerReport
	 * 		{description}
	 */
	public static function addContent($content, $type = self::CONTENT_DATA, $holder = "", $method = self::REPLACE_METHOD, $forceHolder = FALSE)
	{
		$report = self::getReportContent($type, $content, $holder, $method, $forceHolder);
		self::$reports[] = $report;
	}
	...
	protected static function getReport()
	{
		...
		if (logger::status())
		{
			...
			self::addContent($logContext, $type = self::CONTENT_DATA, $holder = "#pageHelper", $method = self::APPEND_METHOD, $force = TRUE);
		}
	 ...
	 * 		{description}
	 */
	protected static function getReportContent($type = self::CONTENT_DATA, $context = NULL, $holder = NULL, $method = self::REPLACE_METHOD, $forceHolder = FALSE)
	{
		$content = array();
		$content['type'] = $type;
		$content['holder'] = $holder;
		$content['method'] = $method;
		$content['force'] = $forceHolder;
