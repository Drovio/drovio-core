...
class template
{
	...
	public function initializeRepository()
	{
		...
		$this->parser = new DOMParser();		
		$root = $this->parser->create("template");
	...
	{ 
		...
		$root = $this->parser->evaluate("//template")->item(0);
			
		// data
		$dataNodeEntry = $this->parser->create($this->xmlRoots['dataRoot']);
		$this->parser->append($root, $dataNodeEntry);
		
		// mapping
		$mapppingNodeEntry = $this->parser->create($this->xmlRoots['mappingRoot']);
		$this->parser->append($root, $mapppingNodeEntry);
			
		//Add settings xml pageStructures root entry
		$pageStructureEntry = $this->parser->create($this->xmlRoots['pageStructureRoot']);
		$this->parser->append($mapppingNodeEntry, $pageStructureEntry);
	...
	{		
		$settingsRoot = $this->parser->evaluate("//"."template")->item(0);		
	...
	public function deploy($deployDest)
	{
		...
		$status = folderManager::copy($this->workingFolder, $deployDest);
	...
	{
		$settingsRoot = $this->parser->evaluate("//".$this->xmlRoots['mappingRoot'])->item(0);	
	...
	{
		$mappingRoot = $this->parser->evaluate("//".$this->xmlRoots['mappingRoot'])->item(0);
			
		$themesArray = array();
		$themeRoot = $this->parser->evaluate($this->xmlRoots['themeRoot'], $mappingRoot)->item(0);
	...
	{
		$gSelector = 'div[data-scope=1]';
		$eSelector = 'div[data-editable=1]';
	
		$parser = new DOMParser();
		$parser->loadContent($structure, DOMParser::TYPE_XML);
		
		$errors = 0;
		// A gloabal area cannot contain another global area
		$globals = $parser->evaluate("//".$gSelector );
		foreach($globals as $g)
		{
			$count = $parser->evaluate($gSelector, $g);
			if($count > 0)
				$errors++;
		}
		
		// An editable area cannot have childrens
		$editables = $parser->evaluate("//".$eSelector);
		foreach($editables as $e)
		{
			$count = $parser->evaluate('div', $e);
			if($count > 0)
				$errors++;
		}
