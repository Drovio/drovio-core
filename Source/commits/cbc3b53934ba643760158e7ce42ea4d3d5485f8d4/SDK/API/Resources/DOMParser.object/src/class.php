...
class DOMParser extends DOMPrototype
{
	...
	public function load($path, $rootRelative = TRUE, $preserve = FALSE)
	{
		...
		{
			// Log
			loggr::log("Loading file '".$file."'.", loggr::INFO, "", loggr::DISK_R);
	...
	public function save($path, $fileName = "", $format = FALSE)
	{
		...
		// Log and save
		loggr::log("Saving file '".$this->filePath."'.", loggr::INFO, "", loggr::DISK_W);
		$this->document->formatOutput = $format;
		$saveFlag = $this->document->save($this->filePath);
		if (is_bool($saveFlag))
		{
			// Log
			loggr::log("Error writing to '".$this->filePath."'.", loggr::ERROR);
		...
		{
			// Log
			loggr::log($saveFlag." Bytes written to '".$this->filePath."'.", loggr::INFO);
