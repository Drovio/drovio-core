...
jq(document).one("ready.extra", function(){
	...
	(function(jq){
		...
		function fileExplorer() {
			...
			jq(document).on("click.fileExplorer", ".filePreviewWrapper .fpClose", function(ev){
				ev.stopPropagation();
		...
		function previewIcons(subPath, activeIcons){
			activeIcons.each(function(){
				...
				//console.log(data);
				JSONServerReport.request(
					"/ajax/resources/sdk/fileExplorer/thumb.php", 
					"GET", 
					data, 
					jqthis,
					function (response) {
						var context = (!response ? undefined : response[0].context);
						
						var jqwrapper = jq(context.htmlContent);
			...
			if (content.filter("pre").length > 0 || content.filter("div:not(:empty)").length)
				...
				jq(window).on("resize.fileExplorer",function() {
					...
					if (hres >= hMax && wres <= wMax){
						...
						//prevWrapper.css("line-height", hres+"px");
					} else if(wres > wMax) {
		...
		function requestContents(subpath) {	
			...
			/*var jqxhr = HTMLServerReport.request(
		...
		function createFolder(subpath, folderName, rowIdentifier) {	
			...
			JSONServerReport.request(
				...
				function (response) {
					var context = (!response ? undefined : response[0].context);
					folderCheck.call(this, context, folderName, rowIdentifier);
				},
				null,
				null
			);
		...
		function renameFile(subpath, oldName, newName, rowIdentifier) {	
			...
			JSONServerReport.request(
				...
				function (response) {
					var context = (!response ? undefined : response[0].context);
					renameCheck.call(this, context, oldName, rowIdentifier);
				},
				null,
				null
			);
		...
		function dropFile(subpath, fileNames, rows) {
			...
			JSONServerReport.request(
				...
				function (response) {
					var context = (!response ? undefined : response[0].context);
					dropCheck.call(this, context, rows);
				},
				null,
				null
			);
		...
		function getFolderView(prettyName, subPath, copy) {
			...
			glist.trigger("getSelectedRows", function(rows) {
				...
				HTMLServerReport.request(
					"/ajax/resources/sdk/fileExplorer/copyMoveDialog.php", 
					"GET", 
					data, 
					fView,
					null,
				...
				JSONServerReport.request(
					...
					function (response) {
						...
						if (!context || !context.contents
							|| (list !== undefined && JSON.stringify(context.contents) != decodeUnicode(list))){ 
