 ...
 * @updated	January 10, 2015, 14:09 (EET)
 */
class RCPage extends HTMLPageTemplate
{
	...
	private function addResources()
	{
		...
		$resource = $this->addScript(url::resource("/Library/Resources/js/q/jq.jquery.js", "https"));
		$rsrcID = BootLoader::getRsrcID("redback", "jquery");
		BootLoader2::setResourceAttributes("js", $resource, 2, $rsrcID, TRUE);
		$resource = $this->addScript(url::resource("/Library/Resources/js/q/jq.jquery.ba-dotimeout.min.js", "https"));
		$rsrcID = BootLoader::getRsrcID("redback", "jquery.ba-dotimeout");
		BootLoader2::setResourceAttributes("js", $resource, 1, $rsrcID, TRUE);
		...
		BootLoader2::setResourceAttributes("css", $layoutRsrc, 1, $rsrcID, TRUE);
		...
		foreach ($allLibraries as $library)
		{
			...
			foreach ($allPackages as $package)
			{
				...
				if (!$testerStatus)
				{
					$cssResourceUrl = BootLoader2::getResourceUrl(BootLoader2::RSRC_CSS, 1, $library, $package);
					$jsResourceUrl = BootLoader2::getResourceUrl(BootLoader2::RSRC_JS, 1, $library, $package);
				}
				else
				{
				...
				if (!empty($cssResourceUrl))
				{
					...
					BootLoader2::setResourceAttributes("css", $resource, 1, $rsrcID, !$testerStatus);
				}
				...
				if (!empty($jsResourceUrl))
				{
					...
					$rsrcID = BootLoader2::getRsrcID($library, $package);
					BootLoader2::setResourceAttributes("js", $resource, 1, $rsrcID, !$testerStatus);
				}
			}
		}
	}
