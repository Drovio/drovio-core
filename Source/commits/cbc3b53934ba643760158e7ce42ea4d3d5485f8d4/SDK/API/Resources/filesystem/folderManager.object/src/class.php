 ...
 * @revised	December 16, 2013, 11:09 (EET)
 */
class folderManager
{
	/**
	...
	public static function create($path, $name = "", $mode = 0777, $recursive = TRUE)
	{
		...
		// Log
		if (!$status)
			loggr::log("Folder '".$folderPath."' failed to be created.", loggr::ERROR, "", loggr::DISK_W);
		else
			loggr::log("Folder '".$folderPath."' created.", loggr::INFO, "", loggr::DISK_W);
	...
	public static function remove($path, $name = "", $recursive = FALSE)
	{
		...
		{
			loggr::log("Folder '".$directory."' doesn't exist to be removed.", loggr::NOTICE, "", loggr::DISK_R);
			return FALSE;
		}
		
		if (!$status)
			loggr::log("Folder '".$directory."' failed to be removed.", loggr::ERROR, "", loggr::DISK_W);
		else
			loggr::log("Folder '".$directory."' removed.", loggr::INFO, "", loggr::DISK_W);
	...
	public static function copy($source, $destination, $contents_only = FALSE)
	{
		...
		{
			loggr::log("Copy to subfolder is not yet supported. Aborting...", loggr::ERROR);
			return FALSE;
		}
		...
		{
			loggr::log("Source or destination are not directories. Aborting...", loggr::ERROR, "", loggr::DISK_R);
			return FALSE;
		}
		...
		// Log
		loggr::log("Folder '".$source."' copied to '".$destination."'.", loggr::INFO, "", loggr::DISK_W);
	...
	public static function move($source, $destination)
	{
		...
		if (!strncmp($destination, $source, strlen($source)))
		{
			loggr::log("Move to subfolder is not yet supported. Aborting...", loggr::NOTICE);
			return FALSE;
		}
		...
		// Log
		loggr::log("Folder '".$source."' moved to '".$destination."'.", loggr::INFO, "", loggr::DISK_W);
	...
	private static function checkPermissions($directory)
	{
		...
		{
			loggr::log("Outer directory doesn't exist or has restricted permissions. Aborting...", loggr::ERROR);
			return FALSE;
		}
