 ...
 * @updated	September 29, 2015, 12:26 (EEST)
 */
class literal extends literalController
{
	...
	public static function get($projectID, $scope, $name = "", $attributes = array(), $wrapped = TRUE, $locale = NULL)
	{
		...
		if ($wrapped)
			return parent::wrap($projectID, $scope, $name, $value, $translationLock, $locale);
		else
			return $value;
	...
	public static function add($projectID, $scope, $name, $value, $description = "")
	{
		...
		if ($status)
			self::$translatedLiterals[$projectID][$scope][$locale][$name] = $value;
	...
	public static function remove($projectID, $scope, $name)
	{
		...
		if ($status)
		{
			unset(self::$translatedLiterals[$projectID][$scope][$locale][$name]);
			unset(self::$nonTranslatedLiterals[$projectID][$scope][$locale][$name]);
		}
	...
	/**
	 * Remove a literal value from the system.
	 * 
	 * @param	integer	$projectID
	 * 		The project id.
	 * 
	 ...
	 * 
	 * @param	string	$locale
	 * 		The locale to remove the value for.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure.
	 */
	public static function removeValue($projectID, $scope, $name, $locale)
	{
		// Remove the literal
		$status = parent::removeValue($projectID, $scope, $name, $locale);
