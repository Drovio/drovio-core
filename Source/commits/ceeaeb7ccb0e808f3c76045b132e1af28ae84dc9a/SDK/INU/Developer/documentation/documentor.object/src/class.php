 ...
 * @revised	July 2, 2014, 10:48 (EEST)
 * 
 * @deprecated	Generally deprecated class.
 */
class documentor extends UIObjectPrototype
{
	...
	public function build($id = '', $document = '')
	{
		...
		if(empty($document))
		{
			$notFoundAnnotation = DOM::create("div", "Could not find manual please reload object", "", "");
			DOM::append($holder, $notFoundAnnotation);
			return $this;
		}
	 ...
	 * 		{description}
	 */
	public function getPresenter($type = self::PLAIN, $fid = '')
	{
		// Clear Report
		HTMLServerReport::clear();
		
		$content = $this->buildPresenter($type, '', '', TRUE);
		DOM::appendAttr($content, "data-frmTarget", $fid);
