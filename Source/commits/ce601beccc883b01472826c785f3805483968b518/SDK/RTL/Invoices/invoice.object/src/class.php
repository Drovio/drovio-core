...
importer::import("RTL", "Invoices", "invoicePrint");
 ...
 * @updated	September 24, 2015, 22:32 (EEST)
 */
class invoice
{
	/**
	...
	public function close()
	{
		...
		$cust = new customer($customerID);
		$newBalance = $cust->getBalance() + $this->getBalance();
	...
	public function exportPDF($shared = FALSE)
	{
		...
		// Initialize invoice print manager
		$invp = new invoicePrint($this->invoiceID);
		
		// Export invoice
		$filePath = $this->getFilePath($type = "pdf");
		return $invp->exportPDF($filePath, $shared);
	}
