 ...
 * @revised	July 18, 2014, 5:16 (EEST)
 */
class fileExplorer extends UIObjectPrototype
{
	...
	public function drop($fileNames, $subPath = "")
	{
		...
		foreach ($fileNames as $key => $file) {
			...
			if (is_dir($path.$file."/"))
			{
				if (directory::isEmpty($path.$file."/"))
					$status[] = folderManager::remove($path, $file."/");
				else
	...
	public function previewFile($fileName, $subPath = "")
	{
		...
		try
		{
			...
			if (is_null($element)){
				...
				$type = $this->getFileType($fileName, $subPath);
				// Find icon class
				if (!empty($type))
					DOM::appendAttr($iconPrev, "class", $type."fi");
				$ext = $this->getFileType($fileName, $subPath, TRUE);
				DOM::attr($iconPrev, "data-ext", substr($ext, 0, 4));
				DOM::attr($iconPrev, "title", $type.(empty($ext) ? "" : " (.".$ext.")"));
				DOM::append($prev, $iconPrev);
			}
			else DOM::append($prev, $element);
		} catch (Exception $e)
		{
			DOM::append($prev, DOM::create("div", $e->getMessage()));
		}
		
		$p = systemRoot.$this->rootPath."/".$subPath."/".$fileName;
		
		//Details
		$details = DOM::create("div", "", "", "fpDetailsWrapper");
		...
		// Name
		DOM::append($details, DOM::create("span", basename($fileName, ".".$ext), "", "fpDetailsName"));
		
		// File info
		$fInfo = DOM::create("div", "", "", "fpDetailsFile");
		DOM::append($details, $fInfo);
		DOM::append($fInfo, DOM::create("span", "Type: ".$type.(empty($ext) ? "" : " (.".$ext.")")));
		DOM::append($fInfo, DOM::create("span", "Location: /".(trim($subPath, "/ \t\n\r\0\x0B"))));
		DOM::append($fInfo, DOM::create("span", "Size: ".fileManager::getSize($p, TRUE)));
		...
		if (!empty($imgInfo)){
			...
			DOM::append($iInfo, DOM::create("span", "Dimensions: ".$imgInfo[0]."x".$imgInfo[1]));
			DOM::append($iInfo, DOM::create("span", "Mime: ".$imgInfo['mime']));
			DOM::append($iInfo, DOM::create("span", "Bit Depth: ".$imgInfo['bits']));
	...
	{ 
		...
		$contents = fileManager::get_contents($path);
		...
		if ($extension == "svg")
		{
			...
			DOM::attr($embed, "src", Url::resource($url = $this->rootPath."/".$subPath."/".$fileName));
			DOM::attr($embed, "type", "image/svg+xml");
			DOM::append($wrap, $embed);
		}
		else if (in_array($extension, self::$fileTypes['image']))
		{
	...
	{
		$tile = DOM::create("div", "", "", "stateViewer");
		if (gettype($msg) == "string")
			DOM::nodeValue($tile, $msg);
		else
			DOM::append($tile, $msg);
