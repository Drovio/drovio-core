...
class zipManager
{
	...
	public static function exists($archive)
	{
		...
		{
			logger::log("File '".$archive."' is not a zip archive.", logger::DEBUG);
			return FALSE;
    		}
		$zip->close();
		logger::log("File '".$archive."' is a zip archive.", logger::DEBUG);
		return TRUE;
	}
	...
	public static function create($archive, $contents = array(), $recursive = FALSE, $includeHidden = FALSE)
	{
		...
		if (!is_dir(dirname($archive)))
		{
			...
			{
				logger::log("Parent folder doesn't exists. Aborting...", logger::DEBUG);
				return FALSE;
			}
		}
		...
		if (empty($contents))
		{
			logger::log("No contents to add in archive. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		{
			logger::log("Contents must be passed in an array. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		if (empty($contents))
		{
			logger::log("No files found. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		foreach ($contents as $array)
			if (!is_array($array))
			{
				logger::log("Contents must contain a 'dirs' and/or a 'files' array. Aborting...", logger::DEBUG);
				return FALSE;
			}
		...
		{
			logger::log("Cannot initialize zip '".$archive."'. Aborting...", logger::DEBUG);
			return FALSE;
    		}
		...
		if ($status)
			logger::log("Zip '".$archive."' created successfully.", logger::DEBUG);
		else
			logger::log("Zip '".$archive."' failed to be created.", logger::DEBUG);
	...
	public static function extract($archive, $destination, $recursive = FALSE, $entries = NULL)
	{
		...
		{
			logger::log("Source file or destination folder doesn't exist. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		if ($zip->open($archive) !== TRUE)
		{
			logger::log("Cannot open zip '".$archive."'. Aborting...", logger::DEBUG);
			return FALSE;
    		}
		...
		if ($status)
			logger::log("Zip '".$archive."' extracted successfully in '".$destination."'.", logger::DEBUG);
		else
			logger::log("Zip '".$archive."' failed to be extracted in the specified directory.", logger::DEBUG);
	...
	public static function getDetails($archive, $entry = NULL, $byName = FALSE)
	{
		...
		{
			logger::log("Source file doesn't exist. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		if ($details === FALSE)
			logger::log("Could not acquire details for zip '".$archive."'.", logger::DEBUG);
		else
			logger::log("Details for zip '".$archive."' acquired successfully.", logger::DEBUG);
	...
	public static function append($archive, $files = array(), $innerDirectory = "", $includeHidden = FALSE)
	{
		...
		if (!is_array($files))
		{
			logger::log("Files must be passed in an array. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		if ($status)
			logger::log("Files appended successfully to '".$archive."'.", logger::DEBUG);
		else
			logger::log("Failed to append files to '".$archive."'.", logger::DEBUG);
	...
	public static function createInnerDirectory($archive, $innerDirectoryName, $innerParentDirectory = "")
	{
		...
		if ( $status )
			logger::log("Directory created successfully in '".$archive."'.", logger::DEBUG);
		else
			logger::log("Could not create the directory in '".$archive."'.", logger::DEBUG);
	...
	public static function createInnerFile($archive, $fileContents, $fileName, $innerDirectory = "")
	{
		...
		if (empty($fileName))
		{
			logger::log("Wrong file name. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		if ( $status )
			logger::log("File created successfully in '".$archive."'.", logger::DEBUG);
		else
			logger::log("Could not create the file in '".$archive."'.", logger::DEBUG);
	...
	public static function remove($archive, $contents)
	{
		...
		if ($status)
			logger::log("Files removed successfully from '".$archive."'.", logger::DEBUG);
		else
			logger::log("Failed to remove files from '".$archive."'.", logger::DEBUG);
	...
	public static function rename($archive, $contents, $newNames)
	{
		...
		if ($status)
			logger::log("Files renamed successfully in '".$archive."'.", logger::DEBUG);
		else
			logger::log("Failed to rename files in '".$archive."'.", logger::DEBUG);
	...
	public static function copy($archive, $origins, $destinations)
	{
		...
		if ($status)
			logger::log("Files copied successfully in '".$archive."'.", logger::DEBUG);
		else
			logger::log("Failed to copy files in '".$archive."'.", logger::DEBUG);
	...
	public static function read($archive, $identifier, $typeOfResponse = self::CONTENTS)
	{
		...
		{
			logger::log("Identifier type error. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		if ($contents == FALSE)
			logger::log("Failed to read from '".$archive."'.", logger::DEBUG);
		else
			logger::log("Resource successfully read from '".$archive."'.", logger::DEBUG);
	...
	private static function acquireDetails($zip, $entry = "", $byName = FALSE)
	{
		...
		else
		{
			...
			{
				logger::log("Invalid entry identifier. Aborting...", logger::DEBUG);
				return FALSE;
			}
	...
	{	
		foreach ((array)$contents['dirs'] as $dir)
		{
			...
			{
				logger::log("Failed to create inner directory ".$innerDirectory.$localname.basename($dir).". Aborting...", logger::DEBUG);
				$zip->unchangeAll();
				return FALSE;
			}
		...
		foreach ((array)$contents['files'] as $file)
		{
			if ($zip->addFile($file, $innerDirectory.$localname.basename($file)))
				continue;
			logger::log("Failed to pack ".$innerDirectory.$localname.basename($file).". Aborting...", logger::DEBUG);
			$zip->unchangeAll();
			return FALSE;
		}
	...
	private static function removeFiles($zip, $contents)
	{
		foreach ($contents as $file)
		{
			...
			{
				logger::log("Invalid entry identifier. Remove Incomplete.", logger::DEBUG);
				$zip->unchangeAll();
				return FALSE;
			}
			...
			foreach ((array)$children as $details)
			{
				if ($zip->deleteName($details['name']))
					continue;
				logger::log("Could not delete ".$details['name'].". Aborting...", logger::DEBUG);
				$zip->unchangeAll();
				return FALSE;
			}
		}
	...
	private static function renameFiles($zip, $contents, $newNames)
	{
		...
		{
			logger::log("Number of contents should match with number of new names. Aborting...", logger::DEBUG);
			return FALSE;
		}
		...
		foreach ($contents as $key => $file)
		{
			if (!is_string($file) && !is_int($file))
			{
				logger::log("Invalid entry identifier. Rename Incomplete.", logger::DEBUG);
				$zip->unchangeAll();
				return FALSE;
			}
			...
			if ($file === FALSE)
			{
				logger::log("Entry with identifier '".$identifier."' not found.", logger::DEBUG);
				continue;
			}
			...
			foreach ((array)$children as $details)
			{
				if ($zip->renameName($details['name'], preg_replace('/'.preg_quote($file, '/').'/', $newNames[$key], $details['name'], 1)))
					continue;
				logger::log("Could not rename ".$details['name'].". Aborting...", logger::DEBUG);
				$zip->unchangeAll();
				return FALSE;
			}
		}
	...
	private static function copyFiles($zip, $origins, $destinations)
	{
		...
		foreach ($origins as $key => $file)
		{
			if (!is_string($file) && !is_int($file))
			{
				logger::log("Invalid entry identifier. Copy Incomplete.", logger::DEBUG);
				$zip->unchangeAll();
				return FALSE;
			}
