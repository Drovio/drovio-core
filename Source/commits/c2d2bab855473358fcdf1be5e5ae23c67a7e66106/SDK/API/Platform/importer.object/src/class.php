 ...
 * @updated	August 29, 2015, 22:49 (EEST)
 */
class importer
{
	/**
	...
	public static function import($library, $package = "", $class = "")
	{
		...
		if (self::$secure === TRUE)
			if (!self::validate($library, $package) && !accessControl::internalCall(1, accessControl::CLASS_CHECK) && !accessControl::internalCall(4, accessControl::ARGS_CHECK))
			{
	...
	private function validate($library, $package)
	{
		...
		$protectedPackages = application::getApplicationProtectedPackages();
		
		// Check if the package is in an allows permission package
		// in the application's manifest
		if (isset($protectedPackages[$library][$package]))
			return TRUE;
