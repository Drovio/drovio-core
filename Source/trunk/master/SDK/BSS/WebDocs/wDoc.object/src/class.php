...
class wDoc extends webDoc
{
	...
	{
		// Get document as webDoc
		$document = parent::get($locale);
		if (empty($document))
		{
			// Load and parse html (if exists)
			$docPathHtml = $this->getDocPath($locale, "html");
			$parser = new DOMParser();
			$parser->load($docPathHtml, FALSE);
			$docBody = $parser->evaluate("//div[@class='body']")->item(0);
			$document = html_entity_decode($parser->innerHtml($docBody));
		}
	...
	public function getRootDirectory()
	{
		...
		$serviceFolder = team::getServicesFolder("/WebDocs/", $this->teamID, $systemAppData = TRUE);
