 ...
 * @revised	November 19, 2014, 22:01 (EET)
 */
class wsServer
{
	/**
	 ...
	 * 		The url from where the website is / will be assecible.
	 * 
	 * @return	boolean
	 * 		True on success, false on failure
	 */
	public function updateInfo($description, $serverType, $connectionType = self::CON_TYPE_FTP, $httpdocs = "httpdocs", $siteUrl = "")
	{
		$parser = $this->xmlParser;
		...
		$this->setProperty($server, "siteUrl", $siteUrl);
	...
	private function getServerInfo($serverID)
	{
		...
		$serverInfo['siteUrl'] = $this->getProperty($server, "siteUrl");
