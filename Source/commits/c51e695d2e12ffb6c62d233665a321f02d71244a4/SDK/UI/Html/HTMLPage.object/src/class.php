 ...
 * @revised	September 19, 2013, 12:35 (EEST)
 */
class HTMLPage extends HTMLPageTemplate
{
	...
	private function populateBody($domain, $static)
	{
		...
		if ($access != "user" && $access != "open")
		{
			...
			$modulePageContainer = $page->build($title, "protectedAccess")->get();
		...
		else if ($static)
		{
			...
			$moduleReport = DOM::evaluate('//report')->item(0);
			$moduleContent = DOM::evaluate("//div[@class='content']", $moduleReport)->item(0);
			DOM::replace($moduleReport, NULL);
			
			// Set Inner HTML
			$moduleInnerHTML = DOM::innerHTML($moduleContent);
			DOM::innerHTML($modulePageContainer, $moduleInnerHTML);
		}
		else
		{
			$moduleContainer = new ModuleContainerPrototype($this->moduleID);
			$modulePageContainer = $moduleContainer->build()->get();
			DOM::attr($modulePageContainer, "id", "pageContainer");
		}
