 ...
 * 
 * @version	3.1-5
 * @created	April 18, 2013, 9:21 (BST)
 * @updated	October 20, 2015, 19:13 (BST)
 */
class simpleForm extends Form
{
	 ...
	 */
	public function build($action = "", $defaultButtons = TRUE, $async = TRUE, $fileUpload = FALSE)
	{
		...
		// Create Form
		$form = parent::build($action, $async, $fileUpload)->get();
		HTML::addClass($form, "simpleForm");
	...
	private function buildControls($defaultButtons)
	{
		...
		if ($defaultButtons)
		{
			...
			$submitBtn = $this->getSubmitButton(literal::dictionary("save"));
			DOM::append($row, $submitBtn);
			$resetBtn = $this->getResetButton(literal::dictionary("cancel"));
