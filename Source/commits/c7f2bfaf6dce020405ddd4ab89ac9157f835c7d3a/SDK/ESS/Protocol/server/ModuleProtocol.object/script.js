...
ModuleProtocol = {
	...
	trigger : function(ev, jqSender, method, callType, extraParams, successCallback, errorCallback, completeCallback, loading) { 
		...
		for (var act in ModuleData)
			requestData += "&__REQUEST["+act.toUpperCase()+"]="+encodeURIComponent(ModuleData[act]);
		...
		for (var act in attrData)
			requestData += "&" + act + "=" + encodeURIComponent(attrData[act]);
		...
		// Set request
		return this.request(jqSender, method, requestData, successCallback, errorCallback, completeCallback, loading);
	},
	request : function(jqSender, method, data, successCallback, errorCallback, completeCallback, loading) {
