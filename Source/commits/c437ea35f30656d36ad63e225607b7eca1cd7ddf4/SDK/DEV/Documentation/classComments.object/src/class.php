 ...
 * @revised	July 12, 2014, 15:40 (EEST)
 */
class classComments
{
	/**
	...
	public static function pretifySourceCode($sourceCode, $manual, $library, $package, $namespace)
	{
		...
		$classInfoCommentsText = self::getClassInfoComments($title = $classInfo['title'], $description = $classInfo['description'], $version = $classInfo['version']."-".$classInfo['build'], $created = $classInfo['datecreated'], $revised = $classInfo['daterevised'], $throws = $classInfo['throws'], $deprecated = $classInfo['deprecated']);
...
//		print_r($prettyCode);
	...
	private function getClassStampComments($library, $package, $namespace)
	{
		...
		$classStamp['library'] = (empty($classStamp['library']) ? "{library}" : $classStamp['library']);
		$classStamp['package'] = trim($package);
		$classStamp['package'] = (empty($classStamp['package']) ? "\\" : $classStamp['package']);
		$classStamp['namespace'] = trim($namespace);
		$classStamp['namespace'] = (empty($classStamp['namespace']) ? "\\" : $classStamp['namespace']);
		$classStamp['copyright'] = 'Copyright (C) '.date('Y').' Skyworks SD. All rights reserved.';
	...
	private function getClassInfoComments($title, $description, $version, $created, $revised, $throws = "", $deprecated = "")
	{
		...
		$classInfo['version'] = (empty($classInfo['version']) ? "0.1-1" : $classInfo['version']);
		$classInfo['created'] = trim($created);
		if (is_numeric($classInfo['created']))
			$classInfo['created'] = intval($classInfo['created']);
		$classInfo['created'] = (empty($classInfo['created']) ? date("F j, Y, G:i (T)") : date("F j, Y, G:i (T)", $classInfo['created']));
		$classInfo['revised'] = trim($revised);
		if (is_numeric($classInfo['revised']))
			$classInfo['revised'] = intval($classInfo['revised']);
		$classInfo['revised'] = (empty($classInfo['revised']) ? date("F j, Y, G:i (T)") : date("F j, Y, G:i (T)", $classInfo['revised']));
	...
	private function getPropertyComments($description, $type)
	{
		...
		$property['type'] = (empty($property['type']) ? "{type}" : $property['type']);
