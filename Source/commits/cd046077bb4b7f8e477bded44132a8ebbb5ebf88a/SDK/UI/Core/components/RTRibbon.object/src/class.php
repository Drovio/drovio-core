 ...
 * @updated	February 25, 2015, 14:44 (EET)
 */
class RTRibbon extends UIObjectPrototype
{
	...
	public function build()
	{
		...
		$rtribbon = DOM::create('div', "", "", "rtribbon");
		DOM::append($holder, $rtribbon);
		
		// Collection's Group
		$this->colGroup = DOM::create('div', "", "", "colGroup");
		DOM::append($rtribbon, $this->colGroup);
		
		// Collection's status/title bar
		$statusbar = DOM::create("div", "", "", "statusbar title");
		DOM::append($rtribbon, $statusbar);
	 ...
	 */
	public function addCollection($collectionID, $title = "", $moduleID = "", $viewName = "", $startup = FALSE, $attr = array())
	{
		// Ribbon Collection
		$collection = $this->createCollection($collectionID, $title, $moduleID , $viewName, $startup, $attr);
	 ...
	 * 		The collection element.
	 */
	public static function createCollection($collectionID, $title = "", $moduleID = "", $viewName = "", $startup = FALSE, $attr = array())
	{
		// Ribbon Collection
		if (!empty($moduleID))
		{
			$mdlContainer = new ModuleContainerPrototype($moduleID, $viewName);
			$collection = $mdlContainer->build($attr, $startup, $collectionID, $loading = FALSE)->get();
