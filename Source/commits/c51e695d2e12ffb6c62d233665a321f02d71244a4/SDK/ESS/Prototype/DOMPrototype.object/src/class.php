 ...
 * @revised	September 17, 2013, 15:05 (EEST)
 */
abstract class DOMPrototype
{
	/**
	 * Adds or gets an attribute from a DOMElement.
	 * Returns FALSE if element is NULL or attribute doesn't match the restrictions.
	 * Returns the attribute value if value not given.
	 * 
	 ...
	 * 		If the value is NULL (or not given), it returns the value of the attribute.
	 * 		If the value is empty (empty string), the attribute is removed.
	 * 
	 * @return	mixed
	...
	public static function attr($elem, $name, $val = NULL)
	{
		...
		// If value is null (no value given), return attribute value
		if (is_null($val))
	...
	public static function innerHTML($element, $value = NULL, $faultTolerant = TRUE)
	{
		...
		else
		{
			...
			{ 
				...
				//logger::log("Ill Formated ".$f->saveXML(), logger::DEBUG);
