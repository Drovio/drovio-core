...
toolbar = {
	...
	loadItems : function() {
		// Empty page nav items items first
		if (jq(".nav_item").length > 0)
			toolbar.clear();
		...
		jq(".nav_item").each(function(ev) {
			...
			jq_this.find('.tlbNavItem').each(function() {
				var jqItem = jq(this);
				var this_index = jqItem.data('instr').index;
				jqItem.data('index', this_index);
				var inserted = false;
				...
				// Insert item in proper position
				jq('.tlbNavItem', jq('.tlbNav.leftNav')).each(function() {
					if (jq(this).data('index') > this_index)
					{
						jq(this).before(jqItem);
						inserted = true;
						return false;
					}
				});
				// Append last item
				if (jq('.tlbNavItem', jq('.tlbNav.leftNav')).length == 0 || !inserted)
					jqItem.removeAttr('data-instr').appendTo(jq('#pageNav'));
			});
			
			// Insert each collection into collection group
			jq_this.find('.collection').each(function() {
				var cr_col_id = jq(this).attr('id');
				// Escape if there is already a collection with the same id
				if (jq('#'+cr_col_id+'.collection').length > 0)
					return true;
