 ...
 * @revised	April 12, 2014, 14:48 (EEST)
 */
class account
{
	/**
	...
	public static function login($username, $password, $rememberDuration = 0)
	{
		...
		self::updateSessionData($accountData, $rememberDuration, TRUE);
	...
	public static function validate()
	{
		...
		if (empty($accountSessionExists))
			self::updateSessionData(NULL, 0, TRUE);
	...
	private static function createSession($accountID, $salt, $personID = "", $rememberDuration = 0)
	{
		...
		$attr['duration'] = $rememberDuration;
		$result = $dbc->execute($q, $attr);
	 ...
	 * @return	void
	 */
	private static function updateSessionData($accountData = NULL, $rememberDuration = 0, $loggedIn = FALSE)
	{
		$accountSessionExists = session::get("exists", "", "account");
		if (!$accountSessionExists)
		{
			...
			if ($loggedIn)
			{
				...
				$attr['sid'] = self::getSessionID();
				$result = $dbc->execute($q, $attr);
				$sessionData = $dbc->fetch($result);
				
				$rememberDuration = $sessionData['duration'];
