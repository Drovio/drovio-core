 ...
 * @updated	November 30, 2015, 11:43 (GMT)
 */
class application extends project
{
	...
	public function publish($version, $branchName = vcs::MASTER_BRANCH)
	{
		...
		foreach ($styles as $styleName)
		{
			...
			$appCssFileContent = BootLoader::resolveURLs($appID, $appCssFileContent, $version, $protocol = NULL);
			BootLoader::exportCSS($appID, "Library", $styleName, $appCssFileContent, $version);
		}
		...
		foreach ($scripts as $scriptName)
		{
			...
			BootLoader::exportJS($appID, "Library", $scriptName, $appJsFileContent, $version);
		}
		...
		foreach ($allViews as $folderName => $views)
			foreach ($views as $viewName)
			{
				...
				$viewCSS = BootLoader::resolveURLs($appID, $viewCSS, $version, $protocol = NULL);
				BootLoader::exportCSS($appID, "Views", $folderName."/".$viewName, $viewCSS, $version);
				
				// Resolve urls in js, minify and export
				$viewJS = jsParser::format($viewJS);
				$viewJS = BootLoader::resolveURLs($appID, $viewJS, $version, $protocol = NULL);
				BootLoader::exportJS($appID, "Views", $folderName."/".$viewName, $viewJS, $version);
		...
		foreach ($packages as $package)
		{
			...
			// Resolve urls in css, format and export
			$packageCSS = BootLoader::resolveURLs($appID, $packageCSS, $version, $protocol = NULL);
			$packageCSS = cssParser::format($packageCSS, $compact = TRUE);
			$packageCSS = BootLoader::resolveURLs($appID, $packageCSS, $version, $protocol = NULL);
			BootLoader::exportCSS($appID, "Source", $package, $packageCSS, $version);

			// Resolve urls in js, minify and export
			$packageJS = jsParser::format($packageJS);
			$packageJS = BootLoader::resolveURLs($appID, $packageJS, $version, $protocol = NULL);
			BootLoader::exportJS($appID, "Source", $package, $packageJS, $version);
		}
