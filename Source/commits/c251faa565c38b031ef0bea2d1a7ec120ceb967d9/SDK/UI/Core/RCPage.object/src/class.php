 ...
 * @updated	March 6, 2015, 11:15 (EET)
 */
class RCPage extends HTMLPageTemplate
{
	...
	public function build($subdomain = "", $dynamic = FALSE)
	{
		...
		$secureSubs[] = "my";
		$secureSubs[] = "admin";
		$secureSubs[] = "developer";
		
		// Check for https protocol
		$uInfo = url::info();
		if ($uInfo['protocol'] == "http" && in_array($uInfo['sub'], $secureSubs))
		{
			// Get current url and transform to https
			$redirectUrl = str_replace("http", "https", $uInfo['url']);
	...
	private function addResources()
	{
		...
		BootLoader::setResourceAttributes("css", $layoutRsrc, coreProject::PROJECT_TYPE, $rsrcID, TRUE);
