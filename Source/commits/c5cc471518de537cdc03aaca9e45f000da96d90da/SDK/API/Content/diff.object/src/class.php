 ...
 * Diff
 * 
 * Given two strings (text) finds and annotates the diferences between
 * 
 * @version	{empty}
 * @created	May 13, 2014, 20:33 (EEST)
 * @revised	May 17, 2014, 15:32 (EEST)
 */
class diff
{
	/**
	...
	/**
	 * Holds possible errors during a functions executio
	 * 
	 * @type	string
	 */
	public $error = '';
	 ...
	 * @return	void
	 */
	public function init($originalText ='', $alteredText = '')
	{
		if(empty($originalText) && empty($alteredText))
			return FALSE;
	 ...
	 * 
	 * @param	char	$mode
	 * 		The type by which the comparison will be made. Use MODE_ constants to set this values. The default is MODE_CHAR
	 * 
	 * @param	boolean	$generatePatch
	 * 		Flag which indicates if a patch (array) will be also be constructed for each change
	 * 
	 * @return	mixed
	 * 		The array of the changes between the two strings or null on failure
	 */
	public function diff($mode = self::MODE_CHAR, $generatePatch = TRUE) 
	{
		...
		// Depending the parameters we are givin in splitString, defining the end of a line and the end of the requested unit(word)
		...
		for($b = $a = 0; $b < $originalLength && $a < $alteredLength ;)
		{
			...
			{
				// $pa is a secondary carret to transverse a small area of  the "altered text"
	...
	/**
	 * Applies the patch creates previously, using diff funtion, on the original text and return the result
	 * 
	 * @param	array	$difference
	 * 		An array of arrays (a list of changes), each one containing information about a change. The nested arrays must contain the 'patch' property.
	 * 
	 * @return	string
	 * 		{description}
	 */
	...
	/**
	 * Splits a string by word or line according to seperator indicator and end (line) indicator
	 * 
	 * @param	string	$string
	 * 		The string to be splited
	 * 
	 * @param	string	$separators
	 * 		The seperators indicators
	 * 
	 * @param	string	$end
	 * 		The end line indicators
	 * 
	 * @param	pointer	$positions
	 * 		(By reference) The positions table which hold the positions that the string was splited
	 * 
	 * @return	array
	 * 		{description}
	 */
	private function splitString($string, $separators, $end, &$positions)
	{
		...
		for($p = 0; $p < $lenght;)
		{
			// Find the number sequential character in $string until a character from $separators.$end is found starting from $p
