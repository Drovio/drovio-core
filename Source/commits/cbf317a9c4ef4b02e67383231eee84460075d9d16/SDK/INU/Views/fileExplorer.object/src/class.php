 ...
 * @revised	August 29, 2014, 13:05 (EEST)
 */
class fileExplorer extends UIObjectPrototype
{
	...
	private static $domTypes = array(
		...
		"pre" => array("js", "php", "css", "xml", "html", "txt", "rtf"),
	...
	public function __construct($rootPath, $id, $friendlyRootName = "", $showHidden = FALSE, $readOnly = TRUE)
	{
		...
		if (trim($rootPath, "/ \t\n\r\0\x0B") == "" || !is_dir(systemRoot.$this->rootPath))
			$this->rootPath = NULL;	
	...
	public function build($subPath = "")
	{
		...
		if (empty($this->rootPath) || !is_dir(systemRoot.$this->rootPath))
			return $this;
