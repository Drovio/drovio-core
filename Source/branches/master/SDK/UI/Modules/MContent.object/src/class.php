 ...
 * 
 * @version	4.0-5
 * @created	June 23, 2014, 10:34 (BST)
 * @updated	November 30, 2015, 11:08 (GMT)
 */
class MContent extends HTMLContent
{
	...
	/**
	 * Get a literal for the current running module.
	 * 
	 * @param	string	$name
	 * 		The literal name.
	 ...
	 * @return	mixed
	 * 		The literal value.
	 * 		See moduleLiteral for more information.
	 */
	public function getLiteral($name = "", $attr = array(), $wrapped = TRUE)
	{
		return moduleLiteral::get($this->moduleID, $name, $attr, $wrapped);
	}
	 ...
	 * 
	 * @param	string	$viewName
	 * 		The module's view name to load.
	 * 		If empty, get the module view that match the module's name.
	 * 		It is empty by default.
	 * 
	 * @return	mixed
	 * 		The module view return content.
	 */
	public function loadView($moduleID, $viewName = "")
	{
		// Load view and return the content
		return module::loadView($moduleID, $viewName);
	}
	 ...
	 * 
	 * @param	string	$name
	 * 		The module query name.
	 * 
	 * @param	integer	$moduleID
	 * 		The module id to get the query from.
	 * 		Leavy empty for current module.
	 * 		It is empty by default.
	 * 
	 * @return	mixed
	 * 		The module query object.
	 */
	public function getQuery($name, $moduleID = "")
	{
		$moduleID = (empty($moduleID) ? $this->moduleID : $moduleID);
		return module::getQuery($moduleID, $name);
	}
	 ...
	 * 
	 * @param	array	$attr
	 * 		A set of attributes to be passed to the module as name => value.
	 * 
	 * @param	string	$class
	 * 		The weblink extra class attribute.
	 * 
	 * @return	DOMElement
	 * 		The DOMElement object.
	 */
	public function getWeblink($href, $content = "", $target = "_self", $moduleID = "", $viewName = "", $attr = array(), $class = "")
	{
		// Get the weblink
		$weblink = parent::getWeblink($href, $content, $target, $class);
		
		// Set module action if any
		if (!empty($moduleID))
		{
			$actionFactory = $this->getActionFactory();
			$actionFactory->setModuleAction($weblink, $moduleID, $viewName, "", $attr);
		}
	 ...
	 */
	public function getReport($holder = "", $method = parent::REPLACE_METHOD)
	{
	...
	/**
	 * Adds a header to the report including all the module resources (css and js) of the loading module and the connected  to this.
	 * 
	 * @return	void
	 */
	private function setModuleResource()
	{
