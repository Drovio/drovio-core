...
ribbon = {
	...
	init : function() {
		...
		jq(document).on("content.modified", function() {
			...
			jq(".uiTlbRibbon .rtribbon .collection").each(function(){
				jq(this).data("title", jq(this).data("title")).removeAttr("data-title");
			});
			
			// Check if ribbon is cloned and re-clone
			if (jq(".rtribbon").length > 1) {
				// Get initial ribbon
				var jqToolbarRibbon = jq(".uiTlbRibbon .rtribbon");
				
				// Get cloned ribbon
				var jqClonedRibbon = jq(".rtribbon").not(".uiTlbRibbon .rtribbon");
	...
	showCollection : function(id) {
		...
		jq(".rtribbon .statusbar").html(collectionTitle);
		if (collectionTitle == "" || collectionTitle == undefined)
			jq(".rtribbon .statusbar").empty();
	...
	showPopup : function(jqNavItem) {
		...
		jqNavItem.popup(jq('.rtribbon', '.uiMainToolbar > .uiTlbRibbon').clone(true).find('.collection').css("min-width","0px").end());
	},
