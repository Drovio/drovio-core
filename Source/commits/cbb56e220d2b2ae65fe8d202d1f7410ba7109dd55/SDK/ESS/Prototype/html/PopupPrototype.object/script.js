	...
	var methods = {
		init : function(settings, extra, callback){		
			return this.each(function(){
				// This timeout is important. It lets the Garbage Collector do its job, 
				// before continuing with the creation of the new popup.
				//setTimeout(function(){
				//console.log("2");
...
//console.log("1");
	...
	function popupGCollector(sender, parent) {
		...
		jq('.uiPopup').not(".fnppContainer")
			...
			//console.log("3");
				disposePopup(jq(this));//jq(this).trigger("dispose");
			}
		}); 
	}
	...
	function popup(settings, extra, callback) {
		...
		/*systemPopup.find(".success").closest(systemPopup).addClass("success");
		systemPopup.find(".error").closest(systemPopup).addClass("error");
		systemPopup.find(".warning").closest(systemPopup).addClass("warning");
		systemPopup.find(".info").closest(systemPopup).addClass("info");*/
		...
		/*jq(window).off("resize.popup");
		jq(window).on({
			"resize.popup":function(ev) {
				setTimeout(function(){
					scoutPosition(systemPopup);
				}, 10);
			}
		...
		// TIMEOUTS should be REMOVED from those two.
		...
		if ((!settings.withBackground) && settings.type.match(/obedient/gi)) {
			...
			//console.log("111");
				...
				extra.parentContainer.on("click.popup", function(ev) {
					...
					var popup = jqthis.children(".uiPopup").not(".elevated");
					if (popup.length == 0)
						return;
					//console.log(popup.clone().length);
					//var opts = popup.data("popup-settings");
		...
		if (settings.withBackground) {
			...
			extra.parent.one("click", ".popupOverlay", function(ev){ 
				ev.stopPropagation();
				jq(this).parent().children(".uiPopup").trigger("cancel").trigger("dispose");
				//console.log("3");
			});
		}
		...
		sender.on("destroyed.popup", function(){
			...
			systemPopup.trigger("popup.cancel").trigger("dispose");
			//console.log("4");
		});
		...
		sender.one("dispose.popup", function(ev){
			...
			popup.trigger("popup.cancel").trigger("dispose");
			//console.log("5");
		});
		...
		//systemPopup.trigger("content.modified");
	}
	...
	function createPopup() {
		...
		// Reevaluate - catch "OK button event"
		systemPopup.find(".uiFormButton").not("[data-action]").filter("[data-control='OK']").on("click", function() {
			systemPopup.trigger("popup.ok").trigger("dispose");
			//console.log("6");
		});
		
		// Reevaluate - catch "Cancel button event"
		systemPopup.find(".uiFormButton").not("[data-action]").filter("[data-control='CANCEL']").on("click", function() {
			systemPopup.trigger("popup.cancel").trigger("dispose");
			//console.log("7");
		});
		...
		// dispose on ESC
		jq(document).on("keyup.popup", function(ev) {
			if (ev.which == 27) {//ESC
				systemPopup.find(".actionButton").trigger("click.close").end().trigger("popup.cancel").trigger("dispose");
				//console.log("8");
	...
	function timeoutPopup() {
		...
		if (systemPopup.children(".error").length > 0) {
			systemPopup.find(".actionButton").append(jq("<embed id='timeout_svg' src='/Library/Media/Elements/icons/16x16/errorTimeout.svg' type='image/svg+xml' width='16px' height='16px' />"));
		} else if (systemPopup.children(".info").length > 0) {
			systemPopup.find(".actionButton").append(jq("<embed id='timeout_svg' src='/Library/Media/Elements/icons/16x16/infoTimeout.svg' type='image/svg+xml' width='16px' height='16px' />"));
		} else if (systemPopup.children(".warning").length > 0) {
			systemPopup.find(".actionButton").append(jq("<embed id='timeout_svg' src='/Library/Media/Elements/icons/16x16/warningTimeout.svg' type='image/svg+xml' width='16px' height='16px' />"));
		} else if (systemPopup.children(".success").length > 0) {
			systemPopup.find(".actionButton").append(jq("<embed id='timeout_svg' src='/Library/Media/Elements/icons/16x16/successTimeout.svg' type='image/svg+xml' width='16px' height='16px' />"));
		} else {
			//
		}
	...
	function revealPopupBackground(systemPopup) {
		...
		if (settings.withFade) {
			...
			jq(".popupOverlay", extra.parent).css("display", "inline");
		}
	}
	...
	function revealPopup(systemPopup) {
		...
		if (settings.withFade) {
			...
			systemPopup.css("display", "inline");
		}
	...
	function relativeToWindow(systemPopup) {
		...
		// Remove anchors
		systemPopup.css({"top" : "", "bottom" : "", "left" : "", "right" : "", "position" : "fixed", "margin-left":"", "margin-right":"", "margin-top":"", "margin-bottom":""});
	...
	// Old popups
	jq.fn.redPopups = function(method, opts, ext) {

	    // call correct popup
	    
	if (method == "ajaxGlobalPopup") { 
		var sender = jq(opts['sender']);
		//var parent = jq(ext['parent']);
		var content = jq(ext['context']);
		...
		for (s in opts) {
			sender.popup[s] = opts[s];
		}
		for (e in ext) {
			sender.popup[e] = ext[e];
		}
		
	  	return sender.popup(content);
	  } else if (typeof method === 'object' || !method) {
	      jq.error('Please specify a method!');
	    } else {
	      jq.error('Method ' +  method + ' does not exist in jQuery.redPopups');
	    }    
	  
	};

	jq.fn.redPopup = function(sender, context) {
		var s = jq(sender);
		var c = jq(context);
		
		var settings = context.find(".info").data("popup-settings");
		var extra = context.find(".info").data("popup-extra");
		
		var p = jq(document.body);
		if (extra !== undefined && extra.parentid !== undefined) {
			p = jq("#"+extra.parentid).eq(0);
			delete extra.parentid;
		}
		...
		for (i in settings) {
			...
			s.popup[d] = (settings[i] == "1" ? true : settings[i]);
			
		}
		for (j in extra) {
			s.popup[j] = (extra[j] == "1" ? true : extra[j]);
		}
		
		var content = c.find(".package").children();
		c.find(".package").after(jq("<div class='popupContent'></div>").append(content));
