 ...
 * @revised	November 22, 2013, 10:35 (EET)
 */
class ajaxPage
{
	/**
	...
	public function create($name, $directory)
	{
		...
		// Create Index 
		$proceed = $this->createIndex($this->name, $this->ajaxDirectory);
		if (!$proceed)
			return FALSE;
	...
	public function export()
	{
		// Update new vcs item info
		$itemID = $this->getItemID();
		$itemTrunkPath = $this->vcs->getItemHeadPath($itemID);
	...
	public function migrate()
	{
		...
		$itemPath = "/Ajax/".$this->ajaxDirectory."/";
		$itemName = $this->name.".php";
		$itemTrunkPath = $vcs->createItem($itemID, $itemPath, $itemName, $isFolder = FALSE);
		$itemTrunkPath = $vcs->getItemTrunkPath($itemID);
		fileManager::create($itemTrunkPath, "", TRUE);
		fileManager::copy($old_itemTrunkPath, $itemTrunkPath);
	}
}
//#section_end#
?>