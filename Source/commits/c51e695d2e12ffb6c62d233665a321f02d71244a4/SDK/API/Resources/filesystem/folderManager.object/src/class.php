 ...
 * @revised	September 6, 2013, 13:35 (EEST)
 */
class folderManager
{
	/**
	...
	public static function create($path, $name = "", $mode = 0777, $recursive = TRUE)
	{
		...
		if (!$status)
			logger::log("Folder '".$folderPath."' failed to be created.", logger::ERROR);
	...
	public static function remove($path, $name = "", $recursive = FALSE)
	{
		...
		{
			logger::log("Folder '".$directory."' doesn't exist to be removed.", logger::NOTICE);
			return FALSE;
		}
		
		if (!$status)
			logger::log("Folder '".$directory."' failed to be removed.", logger::ERROR);
	...
	public static function copy($source, $destination, $contents_only = FALSE)
	{
		...
		{
			logger::log("Source or destination are not directories. Aborting...", logger::ERROR);
			return FALSE;
		}
